...
# ===== [ConsensusZone 主图] - PRD V1.1 §7.4 =====


async def test_consensus_zone_layer_injected(
    mock_session: AsyncMock,
    mock_bars: None,
    empty_registry: None,
) -> None:
    """[ConsensusZone] - indicators 响应包含 consensus_zone 图层和数据。"""
    result = await indicator_service.compute_all_indicators(
        mock_session, TEST_INSTRUMENT_ID, "1d", "none", bars=250,
    )

    # 验证 layers 中有 consensus_zone
    cz_layers = [layer for layer in result["layers"] if layer.get("renderer") == "consensus_zone"]
    assert len(cz_layers) == 1, "应有一个 consensus_zone 图层"
    cz_layer = cz_layers[0]
    assert cz_layer["pane"] == "price", "consensus_zone 应为主图图层"
    assert cz_layer["layer_name"] == "筹码共识区"

    # 验证 data 中有 consensus_zone
    assert "consensus_zone" in result["data"], "data 应包含 consensus_zone"
    cz_data = result["data"]["consensus_zone"]
    assert "isAvailable" in cz_data, "consensus_zone 数据应包含 isAvailable"
    assert "clusters" in cz_data, "consensus_zone 数据应包含 clusters"
    assert "algorithmVersion" in cz_data, "consensus_zone 数据应包含 algorithmVersion"


async def test_consensus_zone_error_does_not_block_other_layers(
    mock_session: AsyncMock,
    mock_bars: None,
    empty_registry: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """[ConsensusZone] - 计算失败时只记录错误，不影响 MACD/SQZMOM 等其他图层。"""

    async def _fail_compute(*args, **kwargs):
        raise RuntimeError("模拟 ConsensusZone 计算失败")

    monkeypatch.setattr(
        "app.services.consensus_zone_service.compute_and_cache_consensus_zone",
        _fail_compute,
    )
    ...
    # consensus_zone 应在 errors 中
    assert "consensus_zone" in result["errors"], "consensus_zone 失败应记录到 errors"
    assert "模拟" in result["errors"]["consensus_zone"]

    # MACD 和 SQZMOM 应正常存在
    assert "macd" in result["data"], "MACD 不应受 consensus_zone 失败影响"
    assert "sqzmom_lb" in result["data"], "SQZMOM 不应受 consensus_zone 失败影响"

    # consensus_zone 不应在 data 中（因为计算失败）
    assert "consensus_zone" not in result["data"], "失败时不应写入 data"


# ===== [ConsensusZone 数据源独立性] - PRD 纠偏：禁止用 macd_bars =====


async def test_consensus_zone_independent_of_display_count(
    mock_session: AsyncMock,
    mock_bars: None,
    empty_registry: None,
) -> None:
    """[数据源独立性] 改变前端 bars count 不改变同一 as_of 的 ConsensusZone 结果。

    ConsensusZone 使用 daily_bars（固定 250 根日线窗口），不使用 macd_bars
    （随显示 count 截断）。因此 bars=100 和 bars=250 的结果必须一致。
    """
    result_100 = await indicator_service.compute_all_indicators(
        mock_session, TEST_INSTRUMENT_ID, "1d", "none", bars=100,
    )
    result_250 = await indicator_service.compute_all_indicators(
        mock_session, TEST_INSTRUMENT_ID, "1d", "none", bars=250,
    )

    cz_100 = result_100["data"]["consensus_zone"]
    cz_250 = result_250["data"]["consensus_zone"]

    # as_of 相同（都来自 daily_bars 最后一根）
    assert cz_100["asOf"] == cz_250["asOf"], "as_of 应来自 daily_bars，不受 count 影响"
    # timeframe 固定 1d
    assert cz_100["timeframe"] == "1d", "ConsensusZone timeframe 应固定 1d"
    assert cz_250["timeframe"] == "1d", "ConsensusZone timeframe 应固定 1d"
    # clusters 相同
    assert cz_100["clusters"] == cz_250["clusters"], "clusters 不应随 count 变化"
    assert cz_100["isAvailable"] == cz_250["isAvailable"]


async def test_consensus_zone_independent_of_display_timeframe(
    mock_session: AsyncMock,
    mock_bars: None,
    empty_registry: None,
) -> None:
    """[数据源独立性] 切换显示周期不改变 ConsensusZone 定义。

    ConsensusZone 使用 daily_bars + timeframe="1d"，不随用户切换到 15m/1h/1w/1mo 改变。
    切换到 15m 时不会用 15m bars 冒充日线共识数据。
    """
    result_1d = await indicator_service.compute_all_indicators(
        mock_session, TEST_INSTRUMENT_ID, "1d", "none", bars=250,
    )
    result_15m = await indicator_service.compute_all_indicators(
        mock_session, TEST_INSTRUMENT_ID, "15m", "none", bars=250,
    )

    cz_1d = result_1d["data"]["consensus_zone"]
    cz_15m = result_15m["data"]["consensus_zone"]

    # as_of 相同（都来自 daily_bars 最后一根，不来自 macd_bars）
    assert cz_1d["asOf"] == cz_15m["asOf"], (
        "as_of 应来自 daily_bars，不应随显示周期变化"
    )
    # timeframe 固定 1d（不随显示周期变化）
    assert cz_1d["timeframe"] == "1d"
    assert cz_15m["timeframe"] == "1d", "ConsensusZone timeframe 不应随显示周期变化"
    # clusters 相同（同一 daily_bars 输入 → 同一结果）
    assert cz_1d["clusters"] == cz_15m["clusters"], (
        "clusters 不应随显示周期变化（数据源为 daily_bars，非 macd_bars）"
    )
