...
| ALIGN-039 | capture worker 偶发 502 因 page.goto networkidle 超时 | 生产观察 2026-07-08 最近 24h 34 条 capture_jobs failed，复现响应体显示 `Page.goto: Timeout 30000ms exceeded. waiting until "networkidle"`。根因：前端 capture 页面存在长连接/持续轮询时 `networkidle` 永不触发。PR #44 将 `wait_until` 改为 `"load"`，并保留 `data-render-ready="true"` 等待 bars+indicators。 | 部署后观察 monitor 事件图片成功率，连续多条 capture_jobs 中 failed 占比应接近 0%；复现之前失败的 instrument+event 应成功 | P1 |
