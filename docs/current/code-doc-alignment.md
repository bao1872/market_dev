...
| ALIGN-038 | monitor 文字成功但图片缺失（capture token claims 不完整） | 静态代码审查发现 `monitor_batch_service._send_chart_images_via_outbox()` 与 `notification_service.test_channel_latest_event()` 生成 capture token 时只传 `subject/event_id/expires_delta`，未传 `scope/user_id/instrument_id`；而 `get_capture_token_payload` 要求 `scope=stock_detail_capture` 且必须含 `user_id/instrument_id/event_id`，`capture.py` 还校验 `token.instrument_id == path instrument_id`。字段缺失会导致 worker-capture 返回 401/403，`image_url` 为空，后续 image Outbox / MessageDelivery 不生成，但文字通知不受影响。本次 PR 已新增 `app.constants.capture.CAPTURE_SCOPE_STOCK_DETAIL` 常量，修复两个调用方 token 生成，新增 2 个测试文件共 6 个用例覆盖 token claims / image_outbox / capture_jobs FAILED / 不阻塞文字通知。 | 部署后执行单条 smoke：触发 monitor_event 或构造 `[SMOKE_IMAGE]` 单标的事件，验证 capture_jobs.status=SUCCEEDED 且 image_url 非空、Outbox 有 delivery_type=image 且 processed、message_deliveries 有 delivery_type=image、image_upload_status=success、image_key 非空、delivery status=success、飞书实际收到图片 | P1 |
