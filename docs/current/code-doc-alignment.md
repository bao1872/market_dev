...
| ALIGN-032 | 趋势选股页仍以 strategy_results 为主表 | `/strategy-runs/{run_id}/results` 已改为 `strategy_run_items` LEFT JOIN `strategy_results`，返回全量 universe；前端 ScreenerPage 行 key 改用 `instrumentId`，"命中"改名"筛选结果"。待 PR 合并部署后生产验证前端显示全量股票 | 部署后前端默认无筛选时 total 应接近全量 active 股票数 | P1 |
