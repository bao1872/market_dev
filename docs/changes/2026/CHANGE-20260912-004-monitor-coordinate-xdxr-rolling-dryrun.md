...
- 状态：`verified_code`（纯单元测试 443 passed，真实 pytdx 毫秒级 quote 验证通过，零破坏性 DB 写入）。

## 1. 变更背景与解决的问题

在完成 G1~G6 的数据源收口与盘中极速监控重构后，本变更推进完成 Stage G7 与 G8：
...
2. **真实 pytdx 只读 Dry-Run 验证（READ-ONLY）**：
   ...
   - 模拟向上突破 SMC BOS 目标位（1287.91 元），精确触发 `smc_bos_retest` 事件；
