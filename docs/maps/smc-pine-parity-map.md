...
> **[CHANGE-20260718-004 ref 隔离]**：`ref/` 目录下所有文件仅供**人工阅读**，禁止作为运行依赖。
> 生产代码、测试、工具、构建脚本在运行时不得 `import`/`open`/`read`/`glob` `ref/`（详见 AGENTS clause 59/60）。
> SMC 算法真源是生产代码 `smc_pine_core.py`；`ref/smc_user_source.pine` 是参考源（人工对照 Pine 语义）。

| 来源 | 路径 | SHA256 | 行数 | 角色 |
|---|---|---|---|---|
| **用户 Pine 源码** | `ref/smc_user_source.pine`（git 跟踪，`git add -f` 例外） | `0bd3d2ad8819f2dc7a9399f0e869ca3c9eced8100f190aa131aac5fe8191988f` | 843 | **SMC 算法参考源**（人工阅读，非运行依赖；用户原创，授权盘迹商业项目使用） |
| 派生导出文件 | `ref/smc_user_export.pine`（CHANGE-20260718-004：已 `git rm --cached`，不再纳入 git 跟踪） | — | 896 | 派生文件（source 末尾加 18 个隐藏 plot），仅人工阅读，不作为测试输入 |
| 同一份内容副本 | `ref/smc_ref.txt` | 同上 | 843 | 历史路径别名，内容与 `smc_user_source.pine` 完全相同 |
| 生产 Python 核心 | `backend/app/strategy_assets/algorithms/features/smc_pine_core.py` | — | 852 | **唯一 Pine 语义核心**（生产+测试共用，SMC 算法真源） |
| 生产薄包装 | `backend/app/strategy_assets/algorithms/features/smc_indicator.py` | — | — | 委托 `compute_smc_pine`，签名不变 |

**禁止**：读取或引用任何第三方 LuxAlgo Pine 源码；维护第二套近似算法；以"clean-room 翻译"为名绕过 parity 验证；在运行时 `import`/`open`/`read`/`glob` `ref/` 目录下任何文件。
...
| fixture | 状态 | 说明 |
|---|---|---|
| Pine golden CSV | **PENDING** | 等待用户从 TradingView 导出事件/OB CSV（使用 `ref/smc_user_export.pine` 的 26 个隐藏 plot；**[CHANGE-20260718-004]** 该文件已 `git rm --cached`，不在 git 跟踪，仅人工阅读，不作为测试输入） |
...
| **京东方A 000725 项目回归基线** | **已确立**（CHANGE-20260718-004） | SMC 输出 17 events / 21 OB / 2 EQL / swing_bias=1；**项目回归基线**（非 TV golden，非完全对齐），用于验证算法无回归 |

**PINE_PARITY_PENDING**：当前无 Pine 导出的 golden CSV，无法进行输出级完全一致断言。**[CHANGE-20260717-001]** golden 测试本身已修复（EQH 类型直接用 core 输出不误映射、日内时间戳用 `isoformat()` 不压缩、容差 0 严格逐 bar、新增 OB/EQ 端点/全链 3 个测试），但无真实 TV fixture 时仍 skip。代码级修复通过，输出级 parity pending。不伪造 fixture，不声称"完全对齐"。**[CHANGE-20260718-004]** 000725 SMC 输出作为**项目回归基线**（17 events / 21 OB / 2 EQL / swing_bias=1），用于验证算法无回归（输入一致 + 输出序列稳定），**不称 TV golden 或完全对齐**；`PINE_PARITY_PENDING` 保留至 TradingView CSV 提供。
