...
**[Phase 5B-1 / CHANGE-20260909 已修复]**：本地 Vite 下 `/` 由 `LandingPage.tsx` 一次性跳转产品官网入口（DEV 模式 `window.location.replace('/marketing-site/')`，生产空跳转到根 `/`），不再无限刷新；生产环境由 Nginx 精确分流到产品官网根槽位 `/site/index.html`（旧「使用说明」portal 已彻底退役，`/portal/` 一律 301 到根）；若生产 Nginx 误配置进入 SPA，显示稳定入口链接（不跳转当前 URL）。测试：`node --test frontend/src/pages/__tests__/landingPageRoot.test.mjs`（6 测试）。
