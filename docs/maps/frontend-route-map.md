...
| `/market` | `MarketWorkspacePage` | Subscriber/Admin | 行情工作区（**无 K 线**）：工具栏 + 分页表格 `MarketStockTable`（名称列 sticky、字体主信息≥14px/辅助≥12px）+ 可收起 `EventStatePanel`（展示 MACD/Evidence/`state.evidence`，使用 `useStockContext` → `GET /api/v1/stocks/{symbol}/context`）；URL 状态 `scope=watchlist\|market&query=xxx&page=1&page_size=<DEFAULT>&sort=symbol:asc&selected=xxx&industry=xxx&concept=xxx&state=up`（**无 `debug` 参数**）；`boardsAvailable=false` 时隐藏行业/概念列并禁用筛选；scope=watchlist 用 `useWatchlistMonitorStatus`，scope=market 用 `useMarketStocks` 服务端分页；右栏首次默认收起（`panji:market-right-panel-collapsed:v1`）；UserAppShell 壳层 |
