...
| **[AFC V1 契约计数与展示规则]** Canonical Registry `atomic_fact_contract_v1.json` 严格 14 Core / 10 Aux / 1 Rejected（V1 累计成交量比）；S2 存在；T3/T6 `ui_enabled=false` 不进用户 payload；V3/T5 阈值未确认 → 仅显示比值 +「分类未启用」；S3 严格 0.33/0.67（0.63→MIDDLE）；S7/S8 禁止负距离；近期变化 recentChanges 不属于 V4.13 Core Fact；V1 永不进 payload；单公式 fallback（新快照与旧 summary 共用 `compute_atomic_facts`） | `backend/tests/test_atomic_fact_contract_service.py`（19 纯函数）+ `frontend/src/features/research-context/__tests__/atomic-facts.test.ts`（5 契约）+ `frontend/src/features/market-workspace/__tests__/change010Contract.test.ts`（回归断言 AtomicFactsPanel） |
| **[AFC V1 个股 context API]** `GET /api/v1/stocks/{symbol}/context` 空态返回 14 Core + 默认 Auxiliary（T3/T6 关不计算）；`auxiliaryHidden` 为响应实际返回的 Auxiliary ID（非 T3/T6）；`GET /api/v1/admin/stocks/{symbol}/debug` 返回 `rawDebug` 可追溯底层 feature/factor；as_of 过滤未来 recentChanges；GET 零写入、不覆盖已发布 run | `backend/tests/test_stock_context_atomic_facts.py`（6 集成测试：空态/有快照/零写入/as_of 无未来/summary 持久化+旧 fallback/admin debug 权限） |
