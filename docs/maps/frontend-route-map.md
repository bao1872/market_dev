...
| `/market` | `MarketWorkspacePage` | Subscriber/Admin | 行情工作区（**无 K 线**）：`MarketToolbar`（仅 scope 分段按钮）+ `StrategyDataTable`（DSA 列表，复用 `getTrendSelectionColumns` 列定义，数据来自 `usePublishedRuns` + `useStrategyRunResults` 最新 published DSA run；`scope=market`→`universe=all`，`scope=watchlist`→`universe=watchlist`；列设置支持显示/隐藏/调整顺序/恢复默认/刷新保留，配置复用 `/me/table-view-presets`）+ 可收起 `EventStatePanel`（展示 MACD/Evidence/`state.evidence`，使用 `useStockContext` → `GET /api/v1/stocks/{symbol}/context`）；URL 状态 `scope=watchlist\|market&selected=xxx`（MarketWorkspacePage 管理）+ `sort/dir/keyword/filters/page/page_size`（StrategyDataTable 内置 `screenerUrlState` 管理）；**默认不显示**形态状态/DSA状态/最近事件列（事件只在 `EventStatePanel` 按需展开时加载）；右栏首次默认收起（`panji:market-right-panel-collapsed:v1`），收起时 0 context 请求；UserAppShell 壳层 |
