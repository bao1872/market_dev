...
- `app/research/regime_discovery/` — 无监督候选状态发现 V1（8 文件：data_access 只读引擎/feature_builder 17 特征派生/distribution_audit PSI+Wasserstein+Spearman/preprocessing winsor+RobustScaler+rank+PCA/models MiniBatchKMeans+GMM/stability bootstrap ARI+transition/reporting manifest+CSV+50MB gate）；`scripts/research_regime_discovery.py` CLI 入口；只读 research_feature_matrix_rows + bars_daily，不修改生产
