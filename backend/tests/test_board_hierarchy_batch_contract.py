...
NOTE: ``app.services.board_analysis_service`` was retired in 6786d173
("no production callers"); the current board owner is Unified Review
(``app.api.board_analysis`` projects ``ReviewScopeObservationFact``).  Tests that
exercised the legacy producer / publication pointer were removed — this file now
locks only the still-valid contracts below:

  - taxonomy model registration
  - PIT membership half-open interval
  - Review scope resolution (trade_date + PIT services)
  - configured universes / industry scope
  - board sync history preservation
  - batch_version deterministic identity
"""
