...
- 本地共享数据库目标测试只能在当轮明确授权下执行，并且不构成部署完成；
- stock_core、chip、Review、bootstrap、publish、pointer、withdrawal 等业务数据动作不是代码部署，必须单独授权并在远程运行环境执行。

## 4.1 验证栈仍属于远程部署

- 远程验证栈（`panji-verify` project、验证数据库 `bz_stock_verify_<sha>`、验证前端、验证 Worker）是远程部署的一种形态，仍受本规则所有位置约束：只能在 `panji-prod` 由正式脚本创建与运行，本地只负责发起控制命令和 SSH Tunnel。
