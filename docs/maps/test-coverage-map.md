...
| 时序特征 V1.10 developing swing 字段保留（daily/m15 developing swing 字段存在性 + 用于时序特征卡片展示 developing 结构）+ derived_relation 继续使用 active swing（不回退 confirmed raw + active 缺失返回 null） | `backend/tests/test_temporal_feature_service.py`（V1.10 developing 字段存在性测试 + active derived_relation 测试） |
...
| Swing 摘要卡 V1.9 active swing 字段契约（摘要卡只显示 active 字段 / confirmed pivot 只在明细 / 禁止模糊标签 / 时序位置标签含 developing 或 confirmed 前缀） | `frontend/scripts/contract-tests/structural-state-panel.test.ts`（V1.9 active 摘要卡测试 + confirmed 只在明细测试 + 时序 developing 标签测试） |
