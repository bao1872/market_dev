...
- CHANGE-20260707-047: Feature Snapshot 持久化（自选股监控指标从实时计算切换为盘后快照）
  ...
  - 新增 `backend/app/services/feature_snapshot_service.py`：复用 `_compute_all_factors_for_bars` / `_compute_daily_context` / `_compute_m15_response` / `_compute_derived_relation` / `bollinger()` 不复制公式；point-in-time 截断 `index.date <= trade_date`；upsert 幂等；单股失败写 `degraded_reasons` 不阻断批次
  - 修改 `backend/app/services/after_close_orchestrator.py` 状态机：`quality_gate → feature_snapshot → publishing`，断点恢复路径更新
  - 修改 `backend/app/api/watchlist.py::get_watchlist_monitor_status`：metrics 唯一来自 `summary_payload`，新增 `calculation_status` 三态（SUCCEEDED/WAITING_SNAPSHOT/NO_SNAPSHOT），删除 `MonitorSnapshotService` 实时 fallback 与 `MonitorState.payload` fallback
  - 新增 `backend/scripts/feature_snapshot_backfill.py` 历史回补 CLI 脚本（核心计算复用 service，脚本只做 CLI/dry-run/resume/批量调用）
  ...
  - 测试：58 passed；ruff 新文件零错误；mypy 新文件零错误；一致性检查 PASS
  - 更新 `02-data-api-contracts.md`、`03-jobs-integrations-operations.md`、`04-frontend-ux.md`、`05-testing-acceptance.md`、6 个 maps 文件
