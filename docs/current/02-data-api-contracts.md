...
| Admin | `/admin/*` | Admin 角色 + 审计；含 `GET /admin/worker-heartbeats` 只读心跳视图（health_state 后端计算：fresh<120s / stale 120-600s / stopped≥600s 或 status=stopped）；新增盘后流水线聚合状态端点 `/admin/after-close/pipeline/latest`、`/admin/after-close/pipeline?trade_date=`、`/admin/after-close/pipeline/runs?limit=`、`POST /admin/after-close/pipeline/run`（admin，幂等），响应 `AfterClosePipelineResponse` 含 5 阶段时间线 + watchlist_ready 严格判定 + data_freshness + 最近 100 条 events；`steps[].step` 取值为固定 5 phase key（`market_prep|dsa_compute|quality_gate|feature_snapshot|publishing`，前端以 `PipelinePhaseKey` 联合类型约束），此为契约值（非向后兼容旧 8 内部步骤 key），内部 `orchestrator_status` 状态机与 `job_run_events` 仍保留细粒度步骤名，前后端须同版本原子部署 |
