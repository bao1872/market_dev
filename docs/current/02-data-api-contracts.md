...
| 策略 | `/strategies`, `/strategy-runs` | 只读 released/published 结果；生产策略 key 仅 `dsa_selector` 与 `watchlist_monitor`；BB/VN 作为 `watchlist_monitor` 内部子模块/指标，不作为独立策略 key |
| 指标 | `/instruments/{id}/indicators` | 实时计算并返回 `dsa_selector`、`watchlist_monitor`（内部包含 `bb`、`volume_node` chart layers）与 `macd`；`bars` 最大 4000 |
