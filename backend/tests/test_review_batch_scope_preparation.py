...
def test_vec1_state_projection_equivalence():
    """PERF-IO-1: SQL-side ``state_payload`` projection to ``_REVIEW_STATE_KEYS``
    must not change any Review consumer output.

    ``_load_batch_states`` now builds ``jsonb_build_object`` over only the keys
    Review reads.  A full payload carrying extra non-consumed keys must produce
    byte-identical ``previous_state_to_flat`` / ``state_to_continuous`` results to
    its projected subset, and every key the consumers actually read must be covered
    by the projection constant (nothing consumed may be dropped).
    """
    ...
    # Full payload: every key Review consumes, plus extras Review must ignore.
    full = {
        ...
        "review_volume_percentile20": 70.0,
        "review_amount_percentile200": 55.0,
        "latest_bos_direction": "up",
        "latest_bos_freshness": 3,
        "latest_choch_direction": "down",
        "latest_choch_freshness": 5,
        "latest_ob_direction": "bullish",
        "latest_ob_freshness": 2,
        "current_vs_prev_volume_mean_ratio": 1.4,
        "prev_segment_volume_mean": 1e8,
        # state_to_continuous() passthrough keys (_CONTINUOUS_STATE_KEYS)
        "regime_strength": 0.7,
        "dsa_dir_bars": 3,
        "dsa_vwap_dev_pct": 0.5,
        "segment_id": 1,
        "segment_direction": 1,
        "segment_bars": 5,
        "segment_change_pct": 2.0,
        "segment_slope": 0.1,
        "current_vs_prev_amount_mean_ratio": 1.3,
        "current_segment_volume_mean": 2e8,
        "active_internal_ob_count": 2,
        "active_swing_ob_count": 1,
        "volatility_phase": "expanding",
        "momentum_direction": "expanding",
        "momentum_change": 0.1,
        "volume_zscore_20": 1.5,
        "available_bars": 200,
        # Keys Review does NOT consume — projection must drop them with no effect.
        "trend_strength": 999,
        "unused_payload": {"nested": True},
        "swing_legs": [1, 2, 3],
    }
    projected = {k: v for k, v in full.items() if k in _REVIEW_STATE_KEYS}

    assert previous_state_to_flat(full) == previous_state_to_flat(projected)
    assert state_to_continuous(full) == state_to_continuous(projected)

    # Boundary guard: every key the consumers actually read is covered by the
    # projection constant; nothing Review consumes may be silently dropped.
    read_keys = set(_CONTINUOUS_STATE_KEYS) | {
        "regime_value", "structure_alignment", "swing_bias", "internal_bias",
        "sqzmom_val", "sqzmom_delta", "volume_ratio_20", "volume_percentile_20",
        "price_position_120d", "review_volume_ratio20", "review_amount_ratio20",
        "review_volume_percentile20", "review_amount_percentile200",
        "latest_bos_direction", "latest_bos_freshness",
        "latest_choch_direction", "latest_choch_freshness",
        "latest_ob_direction", "latest_ob_freshness",
        "current_vs_prev_volume_mean_ratio", "prev_segment_volume_mean",
    }
    assert read_keys <= set(_REVIEW_STATE_KEYS)
