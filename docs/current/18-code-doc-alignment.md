...
| ALIGN-018 | Capture 链路 | Phase C 已实现专用 `/capture/stock/:symbol` 前端路由 + `/api/v1/capture/stocks/{instrument_id}/snapshot` 后端 API + Capture Token 隔离；但 `CaptureStockPage.tsx` 此前仍调用普通端点，`stock_capture_service.py` URL 未携带 `instrument_id`，状态机字段不完整 | 专用 `/capture/stock/:symbol` 路由 + Capture Token 校验 + Snapshot API 唯一数据请求 + URL 携带 `instrument_id` + 完整状态机字段 | `CLOSED` | CHANGE-20260702-011 已重构 `CaptureStockPage.tsx` 仅调用 Snapshot API、`stock_capture_service.py` URL 携带 `instrument_id`、补齐 `capture_status`/`image_upload_status`/`overall_status` 状态机；`tests/test_capture_snapshot.py`、`tests/test_capture_token_isolation.py`、`tests/test_state_machine.py` 通过；frontend contract 52 passed；GitHub Actions run [#36](https://github.com/bao1872/market_dev/actions/runs/28654256829) 最终 HEAD `a053d0c` 全部 blocking jobs 成功；真实飞书生产 E2E 待 Phase F 部署后执行 |
...
| GitHub Actions CI | 已完成 | run [#36](https://github.com/bao1872/market_dev/actions/runs/28654256829) 最终 HEAD `a053d0c` 全部 blocking jobs success |
| 生产部署验证 | 未验证 | 未执行 |
