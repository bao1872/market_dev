...
async def _chip_consensus_poll_once() -> bool:
    """[ChipConsensusWorker] - 单次轮询 façade（W7B1：owner 已迁至 app.services.chip_consensus_worker_poll）。

    真实领取 + 执行逻辑见 app.services.chip_consensus_worker_poll.poll_chip_consensus_once；
    本函数仅做依赖注入后委托，保持 worker 顶层入口名 _chip_consensus_poll_once 不变
    （chip standalone runtime 仍通过 run_chip_consensus_worker 注入 poll_once=此 façade）。
    """
    from app.services.chip_consensus_worker_poll import (
        poll_chip_consensus_once,
    )

    return await poll_chip_consensus_once(
        session_factory=AsyncSessionLocal,
        worker_instance_id=_WORKER_INSTANCE_ID,
        logger=logger,
    )
