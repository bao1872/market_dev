...
def _rich_payload(trade_date: date) -> dict:
    ...
    members = [
        MemberObservation(
            member_id="m1", price_candidate=True, return_1d=2.0, amount=1e6,
            trend=Direction.UP, swing=Direction.UP, internal=Direction.UP,
            momentum=MomentumDirection.EXPANDING, regime_strength=0.3,
            vol_ratio20=1.0, vol_ratio200=2.0,
        ),
        MemberObservation(
            member_id="m2", price_candidate=True, return_1d=-1.0, amount=2e6,
            trend=Direction.DOWN, swing=Direction.DOWN, internal=Direction.DOWN,
            momentum=MomentumDirection.CONTRACTING, regime_strength=0.5,
            vol_ratio20=2.0, vol_ratio200=3.0,
        ),
        MemberObservation(
            member_id="m3", price_candidate=True, return_1d=0.0, amount=3e6,
            trend=Direction.SIDEWAYS, swing=Direction.SIDEWAYS, internal=Direction.SIDEWAYS,
            momentum=MomentumDirection.FLAT, regime_strength=0.7,
            vol_ratio20=3.0, vol_ratio200=4.0,
        ),
    ]
...
def test_primitive_extraction_real_payload() -> None:
    ...
    for key in HISTORICAL_READY_PRIMITIVE_KEYS:
        ...
        fact = compute_position_series(
            [{"trade_date": "2026-08-14", "observation": payload}], key
        )[0]
        assert fact["value"] == expected, key
    # Sanity: the rich payload yields non-trivial (non-degenerate) values.
    facts = compute_historical_positions(
        [{"trade_date": "2026-08-14", "observation": payload}]
    )
...
def test_current_only_primitives_excluded() -> None:
    ...
    payload = _rich_payload(date(2026, 8, 14))
    out = compute_historical_positions(
        [{"trade_date": "2026-08-14", "observation": payload}]
    )
