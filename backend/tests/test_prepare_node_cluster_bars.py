...
测试场景：
1. 正常输入：daily=300/15m=1500/1m=10 → 输出 daily=250/15m=1200/1m=2
...
def test_prepare_node_cluster_bars_normal_input():
    """场景1：正常输入截断到 250/1200/2。"""
    daily = _make_bars(300, datetime(2025, 1, 1), freq="D")
    bars_15m = _make_bars(1500, datetime(2025, 1, 1), freq="15min")
    ...
    assert len(result.bars_15m) == IC.NODE_CLUSTER_LOW_BARS  # 1200
...
def test_prepare_node_cluster_bars_dedup_keep_last():
    ...
    result = prepare_node_cluster_bars(
        daily_dup, _make_bars(1200, datetime(2025, 1, 1), freq="15min"),
...
def test_prepare_node_cluster_bars_truncate_over_limit():
    """场景3：超过 1200 根 15m 数据时只取最后 1200 根。"""
    bars_15m_extra = _make_bars(2000, datetime(2025, 1, 1), freq="15min")
    # 标记最后一根 close 为唯一值
    ...
    assert len(result.bars_15m) == 1200
...
def test_prepare_node_cluster_bars_insufficient_records_actual_count():
    ...
    bars_15m_short = _make_bars(500, datetime(2025, 1, 1), freq="15min")  # < 1200
...
def test_prepare_node_cluster_bars_no_date_column_required():
    ...
    """
    daily = _make_bars(250, datetime(2025, 1, 1), freq="D")
    bars_15m = _make_bars(1200, datetime(2025, 1, 1), freq="15min")
...
def test_prepare_node_cluster_bars_profile_meta_diagnostic_fields():
    ...
    assert meta["input_15m_bars"] == 1200
...
def test_prepare_node_cluster_bars_sort_index_ascending():
    ...
    result = prepare_node_cluster_bars(
        daily_reverse,
        _make_bars(1200, datetime(2025, 1, 1), freq="15min"),
        _make_bars(2, datetime(2025, 6, 18, 9, 30), freq="1min"),
    )
