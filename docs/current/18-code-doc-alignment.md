...
| ALIGN-014 | CI | `ruff`/`type-check` job 曾设置 `continue-on-error: true`，失败不阻断 workflow；`Ruff Changed Files` 相对 `main` 检查整个大型 feature diff 会失败；旧 `type-check` 任务因 252 个历史 mypy 错误直接阻断合并 | release 候选最终 HEAD 的 blocking jobs 全绿，新增文件 Ruff/mypy 零错误，历史债务不新增/不增加/总数不超基线 | `KNOWN_GAP` | 本地已验证三层 Ruff 与三层 Mypy 配置：`Ruff New Files` / `Mypy New Files` 阻断新增 Python 文件，`Ruff Baseline Regression` / `Mypy Baseline Regression` 阻断历史债务新增/增加，`Ruff Full Repository Report` / `Mypy Full Repository Report` 非阻断上传报告；Ruff 基线 commit `64ed75c`、诊断总数 930→903、无新增/增加诊断；Mypy 基线 commit `64ed75c`、诊断总数 242（mypy 2.1.0 + numpy<2.5.0）、当前 241、无新增/增加诊断；已修复 CI 中 mypy 报告步骤因历史错误提前失败的问题，并将 `numpy` 上限收紧为 `<2.5.0` 避免 mypy 2.1.0 与 numpy 2.5.0+ stub 崩溃；GitHub Actions 最终 HEAD 验证中，故保持 `KNOWN_GAP`；见 CHANGE-20260702-010、CHANGE-20260702-011 |
...
| GitHub Actions CI | 修复中 | 已修复 mypy 报告步骤与 numpy 版本问题，正在等待最终 HEAD 的 blocking jobs 结果 |
| 生产部署验证 | 未验证 | 未执行 |
