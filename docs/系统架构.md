...
- **路由模块**：`auth` / `instruments` / `calendar` / `market` / `bars` / `indicators` / `strategies` / `strategy_runs` / `dsa_backfill` / `monitor_states` / `strategy_events` / `notifications` / `admin_membership` / `watchlist` / `stock_memos` / `health` / `metrics`
...
- **外部依赖**：物理机 PostgreSQL 5432（通过 `extra_hosts: host.docker.internal` 访问）
