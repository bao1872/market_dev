...
| 盘后流水线聚合 API（8 场景）：无 run/运行中/成功/watchlist_ready 判定/sample scope 不计入/失败带 error/POST 幂等/events 限 100 条/非 admin 403 | `test_admin_after_close_pipeline.py`（8 个用例） |
