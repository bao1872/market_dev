...
# ===== V1.9 active swing derived_relation =====
def test_derived_relation_uses_active_not_confirmed_raw() -> None:
    """derived_relation 必须使用 active swing [0,1]，不回退 confirmed raw（可能 <0 或 >1）。"""
    daily = {
        "daily_dsa_dir": 1,
        # active 字段存在
        "daily_price_position_in_active_swing_0_1": 0.3,
        # confirmed raw 字段也存在（但不应被使用）
        "daily_price_position_in_swing_0_1": 1.997,
    }
    m15 = {
        "m15_price_position_in_active_swing_0_1": 0.55,
        "m15_price_position_in_swing_0_1": 0.5,
    }
    r = _compute_derived_relation(daily, m15, [])
    # 应使用 active：0.55 - 0.3 = 0.25，而不是 confirmed raw：0.5 - 1.997 = -1.497
    assert r["m15_position_relative_to_daily"] is not None
    assert abs(r["m15_position_relative_to_daily"] - 0.25) < 1e-9

    # 关键测试：active 存在但 confirmed raw 不存在时，仍应返回值
    daily_only_active = {
        "daily_dsa_dir": 1,
        "daily_price_position_in_active_swing_0_1": 0.3,
        # 无 confirmed raw 字段
    ...
    r = _compute_derived_relation(daily_only_active, m15_only_active, [])
    assert r["m15_position_relative_to_daily"] is not None
    assert abs(r["m15_position_relative_to_daily"] - 0.25) < 1e-9


def test_derived_relation_active_null_returns_null() -> None:
    """任一 active 缺失时 derived_relation 返回 null，不回退 confirmed raw。"""
    # m15 active 为 None
    daily = {
        "daily_dsa_dir": 1,
        "daily_price_position_in_active_swing_0_1": 0.3,
        "daily_price_position_in_swing_0_1": 1.997,  # confirmed raw 有值
    }
    m15 = {
        "m15_price_position_in_active_swing_0_1": None,  # active 缺失
        "m15_price_position_in_swing_0_1": 0.5,  # confirmed raw 有值但不应使用
    }
    r = _compute_derived_relation(daily, m15, [])
    assert r["m15_position_relative_to_daily"] is None
    ...
    m15_ok = {"m15_price_position_in_active_swing_0_1": 0.55}
...
def test_derived_relation_uses_active_not_confirmed_raw_full() -> None:
    """derived_relation 用 active swing [0,1]，不用 confirmed raw（可能 <0 或 >1）。

    场景：daily active=0.7, confirmed_raw=1.997
          m15 active=0.55, confirmed_raw=0.5
    期望：m15_position_relative_to_daily = 0.55 - 0.7 = -0.15
    （不是 confirmed raw 0.5-1.997=-1.497）
    """
    daily = {
        "daily_dsa_dir": 1,
    ...
    r = _compute_derived_relation(daily, m15, [])
    # 必须用 active：0.55 - 0.7 = -0.15
    assert r["m15_position_relative_to_daily"] is not None
    assert abs(r["m15_position_relative_to_daily"] - (-0.15)) < 1e-9
