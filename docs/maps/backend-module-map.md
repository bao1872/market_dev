...
| jobs/admin | `api/admin_after_close.py`, admin APIs | scheduler recovery, job services, `after_close_pipeline_service.py`（聚合 pipeline 状态：5 阶段时间线 + watchlist_ready + data_freshness + events） | `scheduler_job_runs`, `worker_heartbeats`, `stock_feature_snapshot_runs`, `job_run_events` | 管理任务与可观察性 |
