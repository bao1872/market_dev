...
class InviteCodeCreate(BaseModel):
    ...
    2. 旧模式（兼容）：提供 plan_code + grant_days，从 plans 表读取 monitor_limit

    capabilities 优先于 plan_code；两者都为默认值时使用旧模式（observe_20 + 1 天）。
    """
    ...
    grant_days: int = Field(
        default=1,
        ge=1,
        le=365,
        description="兑换后增加的天数（1-365，1 = 1 天）",
    )
