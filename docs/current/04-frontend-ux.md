...
- `/market` 渲染 `MarketWorkspacePage`（`frontend/src/features/market-workspace/MarketWorkspacePage.tsx`），**无 K 线**，布局为工具栏 + DSA 列表表格 + 可收起事件状态面板：
  ...
  - **右栏容器与小 K 线（CHANGE-20260713-010）**：`MarketRightPanel`（`frontend/src/features/market-workspace/MarketRightPanel.tsx`）组合 `MiniKlineCard`（顶部）+ `EventStatePanel`（底部），保持 `EventStatePanel` 单一职责不变；`MiniKlineCard`（`frontend/src/features/market-workspace/MiniKlineCard.tsx`）使用 `lightweight-charts` v4 `createChart` + `addCandlestickSeries` 渲染简化 K 线（无指标/Node/事件/工具栏），三按钮"日线/周线/月线"切换 `1d`/`1w`/`1mo`，默认 `1d`；`useMiniKlineData`（`frontend/src/features/market-workspace/useMiniKlineData.ts`）定义 `BARS_COUNT` = `{1d: 80, 1w: 60, 1mo: 48}`，`refetchInterval: false` 禁用轮询；面板收起时由父组件 `MarketWorkspacePage` 不挂载 `MarketRightPanel`，`bars`/`context` 请求均为 0；展开后只请求活动周期（不预取三周期），切回已加载周期使用 React Query 缓存；chart 实例仅创建一次（`useEffect []`），`ResizeObserver` 响应式 + 卸载清理 `disconnect()` + `chart.remove()` + ref 清空，避免重复 canvas；`symbol=null` 时 `MiniKlineCard` 内部显示提示，`EventStatePanel` 不渲染。
