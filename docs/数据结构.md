...
> 自动生成 by tools/update_docs.py | 生成时间: 2026-06-26 21:54:29
...
| status | VARCHAR(16) | 否 | active |  |  | 状态（active/inactive/delisted/suspended）。inactive 由 cleanup_inactive_instruments 标记（长期无日线数据的 active 股票）；覆盖率计算分母只算 status='active' 且匹配 A 股股票代码的标的（见 stock_symbol_sql_filter） |
