...
| indicators | `app/api/indicators.py` | 策略指标实时计算；`bars` 最大 4000，与 bars API 及 Node Cluster 契约对齐；响应 `data.sqzmom_lb` 为全局技术指标，由 `app.strategy_assets.algorithms.features.sqzmom_lb.compute_sqzmom_lb` 计算，前端只渲染不计算；**响应含 `source_bar_times`/`source_bar_hash` 数据源诊断字段**，由 `app.services.chart_bars_service.compute_source_bar_times/hash` 按 `timeframe` 生成（15m/1h 用 `YYYY-MM-DDTHH:MM:SS`，1d 用 `YYYY-MM-DD`）；`indicator_service` 在 15m/1h 必须用 `macd_bars`（当前 timeframe bars）而非 `daily_bars` 计算，与 chart bars 同源；**indicator cache schema 版本**由 `indicator_cache.ALGORITHM_VERSION` 控制（当前 `v4`，PR #31），修改计算逻辑/source 格式/BB 路径必须 bump 版本使旧缓存失效，禁止手动 `DEL` 单只股票 key；**DSA overlay 1d-only by design**（PR #31），15m/1h DSA 按钮 disabled 并提示"DSA VWAP 当前仅支持日线结构锚"；**BB/MACD/SQZMOM overlay 跟随当前 timeframe bars**，15m/1h BB 由 `_adapt_watchlist_bb` 用 `compute_bollinger(macd_bars, length=20, mult=2.0)` 计算，禁止日线阶梯线映射 |
