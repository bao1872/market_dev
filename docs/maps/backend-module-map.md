...
- `app/research/regime_discovery/` — 无监督候选状态发现 V1（8 文件：data_access 只读引擎 + `get_all_matrix_rows` 完整横截面读取（Phase E）/ `get_all_instrument_ids` / feature_builder 17 特征派生（向量化 `groupby().shift()` 时序差分）/ distribution_audit PSI+Wasserstein+Spearman / preprocessing winsor fit/transform 分离 + RobustScaler + 横截面 rank + PCA fit/transform 分离 / models MiniBatchKMeans+GMM + assign_clusters / stability bootstrap ARI+transition+dwell+monthly_prevalence / reporting manifest+9 CSV+50MB gate）；`scripts/research_regime_discovery.py` CLI 入口（sample 阶段拟合 + Phase E 全量 assignment 复用 sample 参数）；只读 research_feature_matrix_rows + bars_daily，不修改生产；`SET LOCAL statement_timeout='600s'` 应对全量读取
