...
| 表 | 语义 |
|---|---|
| `user_table_view_presets` | 用户表格视图配置（保存筛选/排序/列设置 preset）；唯一键 `(user_id, table_id, strategy_key, name)`；索引 `(user_id, table_id, strategy_key)` 用于查询和 quota 检查；config 为 JSONB 仅允许 keyword/sort/filters/hiddenColumns/pageSize（禁止 selectedKeys/page/activeRunId/rows）；每 user+table_id+strategy_key 最多 20 个（应用层 quota）；is_default 同维度至多 1 个 true（应用层互斥更新）；user_id 由认证上下文注入不接受 body 传入；migration 059 创建 |
