...
| market_data | `api/bars.py`, `api/market.py`, `api/calendar.py`, `api/instruments.py` | `market_data_aggregation_service.py`, `bars_coverage_service.py`, `calendar_seed.py`, `market_status_service.py` | bar/instrument/calendar models & repositories | 页面、指标、截图必须同源；覆盖率统一由 `BarsCoverageService` 计算；quote 实时性统一由 `market_status_service.compute_market_session` 判断，仅 `MORNING_SESSION`/`AFTERNOON_SESSION` 触发 pytdx |
