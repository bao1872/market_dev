...
| feature_snapshot | - | `services/feature_snapshot_service.py`, `services/after_close_orchestrator.py` (步骤 3.5), `services/calendar_service.py` (`get_previous_trading_day_async` / `get_most_recent_trading_day_async`) | `models/stock_feature_snapshot.py`, migration `056_stock_feature_snapshots` | 复用 `_compute_all_factors_for_bars` / `_compute_relation` / `_compute_daily_context` / `_compute_m15_response` / `_compute_derived_relation` / `bollinger()` 不复制公式；point-in-time 截断 `index.date <= trade_date`；upsert 幂等；单股失败写 `degraded_reasons` 不阻断批次；失败比例超 30% 抛 `RuntimeError`；盘后 orchestrator 在 `quality_gate` 与 `publishing` 之间执行；`compute_for_trade_date` 不内部 commit，由 caller 决定 commit/rollback |
| feature_snapshot_backfill | - | `scripts/feature_snapshot_backfill.py`（CLI 调用层，无业务计算） | - | 历史交易日批量回补；核心计算复用 `feature_snapshot_service.compute_for_trade_date`；支持 `--start/--end=latest/--batch-size/--resume/--dry-run/--failure-threshold`（`--commit-every` 已移除）；`--resume` 真正跳过已存在 instrument（按完整唯一键过滤，不重新计算）；每个交易日独立事务（成功 commit，失败 rollback）；与 `backend/scripts/backfill_all_periods.py` 同目录同调用方式 |
