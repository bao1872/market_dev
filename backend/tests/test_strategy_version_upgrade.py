...
async def test_seed_creates_new_released_versions(db_session):
    """seed_strategies 在空库上创建 1.1.0/1.4.0 released 版本。"""
    ...
    assert ("dsa_selector", "1.4.0") in result_map, (
        "dsa_selector v1.4.0 必须被 seed_strategies 创建"
    )
    assert result_map[("watchlist_monitor", "1.1.0")] == "released"
    assert result_map[("dsa_selector", "1.4.0")] == "released"
...
@pytest.mark.asyncio
async def test_dsa_selector_version_is_1_4_0(db_session):
    """dsa_selector 最新 released 版本号必须是 1.4.0。"""
    with patch.object(db_session, "commit", new=db_session.flush):
        await seed_strategies(db_session, release=True)
    ...
    assert latest_version.version == "1.4.0", (
        f"dsa_selector 最新 released 版本应为 1.4.0，实际为 {latest_version.version}"
    )
