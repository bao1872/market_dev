...
- `/market` 渲染 `MarketWorkspacePage`（`frontend/src/features/market-workspace/MarketWorkspacePage.tsx`），**无 K 线**，布局为工具栏 + DSA 列表表格 + 可收起事件状态面板：
  ...
  - **MiniKline viewport P0 修复（CHANGE-20260715-001）**：`MiniKlineCard` 使用纯函数 `computeMiniKlineViewport`（`frontend/src/features/market-workspace/miniKlineViewport.ts`）替代 `fitContent` 自动适配；按周期 clamp 可见根数——15m/60m 50-64、日线 48-58、周线 40-52、月线 30-40；右侧留白 3 根 bar；不调用 `fitContent`，避免小 K 线被压缩成单根线或留白过大；viewport 计算不依赖指标/Node/事件渲染，不改变 DSA、Node Cluster、盘中监控或 Capture 计算口径。
...
- **SMC 图层开关（CHANGE-20260715-001）**：`CHART_LAYER_MANIFEST` 新增 `smc` 图层（name="智能资金"，kind="main"，默认 `false`）；仅 `/stock/:symbol` 个股详情请求时携带 `include_smc=true`，`/market` 右栏小 K 线不携带（SMC 只进入个股详情指标链，不进入行情工作区小 K 线）；前端只消费后端 `data.smc` DTO 渲染市场结构关键点位，不重新计算；**FVG 完全排除**（不计算、不返回、不缓存、不渲染）；API 未返回 `smc` 时页面不崩溃；
