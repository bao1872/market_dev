...
| watchlist | `api/watchlist.py` | watchlist service / limit logic / `MonitorSnapshotService` fallback | `user_watchlist_items` model | 到期权限和额度检查；无 MonitorState 时 fallback |
...
| coverage | - | `bars_coverage_service.py` | `bars_daily`, `instruments` | 统一 A 股覆盖率口径，供 scheduler/orchestrator/overview 使用 |
