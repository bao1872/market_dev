...
| ALIGN-030 | 部分标的历史 bars 覆盖不足 | 生产 DB 中 `000001` 仅 5 根日线、66 根 15m（约 6 个交易日），导致个股详情 K 线显示最近几天；`600519`/`300750` 日线 846 根、15m 约 8000 根数据充足 | 完成历史行情回补，或明确排除该类标的；所有页面显示标的需满足 Node Cluster 最小输入 | P1 |
