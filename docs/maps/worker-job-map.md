...
| **qstock board sync（TODO 未接入调度）** | -（无独立 compose 服务） | `market_boards`, `market_board_memberships` | **独立每日任务，尚未接入 scheduler/compose**（TODO）；服务入口 `board_sync_service.sync_boards()`；拉取适配器 `qstock_fetcher.QStockFetcher`（延迟导入 qstock，不成为用户请求链运行时依赖）；流程：拉取板块目录 + 成分 → 暂存集合 → 完整性校验（空集合/目录数≥100/成分数≥3000/异常降幅≤20%）→ 事务原子切换（TRUNCATE+INSERT）；**失败保持上一成功版本，不删除旧关系**；单并发，批量 500-1000；只保存最新关系，不存板块历史/行情/资金流；详见 `current/03-jobs-integrations-operations.md` qstock 板块同步章节 |
