...
//  3. 股票名称筛选独立于顶部 keyword（StrategyDataTable 移除 filterAlias，stock 列用 contains/not_contains/eq）
//  4. 小 K 线：只请求活动周期、收起 0 请求、五周期切换（MiniKlineCard + useMiniKlineData + MarketRightPanel）
...
// =================================================================
// 三、股票名称筛选独立于顶部 keyword（CHANGE-20260714-001）
// =================================================================

test('DataTableColumn 不含 filterAlias 字段（已移除，stock 列改用普通筛选）', () => {
  const src = readSource(DATA_TABLE_PATH)
  assert.ok(
    !src.includes("filterAlias?:"),
    'DataTableColumn 不得含 filterAlias 字段（CHANGE-20260714-001 已移除）',
  )
})

test('StrategyDataTable 不含 KeywordFilterPopover/isKeyword（stock 列用普通 FilterPopover）', () => {
  const src = readSource(DATA_TABLE_PATH)
  // KeywordFilterPopover 组件已移除（不得作为 JSX 使用）
  assert.ok(
    !/<KeywordFilterPopover/.test(src),
    'StrategyDataTable 不得使用 <KeywordFilterPopover>（已移除）',
  )
  // isKeyword 标志已移除
  assert.ok(
    !src.includes('isKeyword'),
    'StrategyDataTable 不得含 isKeyword（filterAlias 已移除）',
  )
  // effectiveKeyword 仍用于顶部搜索（非列筛选）
  assert.ok(
    src.includes('effectiveKeyword'),
    'StrategyDataTable 仍需 effectiveKeyword 用于顶部搜索',
  )
})

test('stock 列不含 filterAlias（改用普通 filterable + contains/not_contains/eq）', () => {
  const src = readSource(join(__dirname, '..', '..', 'trend-selection', 'columns.tsx'))
  assert.ok(
    !src.includes("filterAlias:"),
    'stock 列不得设置 filterAlias（CHANGE-20260714-001 已移除）',
  )
  assert.ok(
    src.includes('filterable: true'),
    'stock 列必须保留 filterable: true（支持 contains/not_contains/eq）',
  )
})

// =================================================================
// 四、小 K 线：只请求活动周期、收起 0 请求、五周期切换（CHANGE-20260714-001）
// =================================================================

test('useMiniKlineData 定义 BARS_COUNT：15m=120, 1h=100, 1d=80, 1w=60, 1mo=48', () => {
  const src = readSource(USE_MINI_KLINE_PATH)
  assert.ok(src.includes('15m') && src.includes('120'), 'useMiniKlineData 15m 必须 120 根')
  assert.ok(src.includes('1h') && src.includes('100'), 'useMiniKlineData 1h 必须 100 根')
...
test('MiniKlineCard 使用 lightweight-charts createChart + CandlestickSeries', () => {
  ...
  const ctrlSrc = readSource(CONTROLLER_PATH)
  assert.ok(src.includes('createChart'), 'MiniKlineCard 必须使用 lightweight-charts createChart')
  // CHANGE-20260715-007: addCandlestickSeries 移至 miniKlineController
  assert.ok(
    ctrlSrc.includes('addCandlestickSeries') || ctrlSrc.includes('CandlestickSeries'),
    'miniKlineController 必须使用 CandlestickSeries 渲染 K 线',
  )
  // CHANGE-20260714-001: 五按钮 15m/60m/日/周/月
  assert.ok(src.includes('15m'), 'MiniKlineCard 必须含"15m"按钮')
  assert.ok(src.includes('60m'), 'MiniKlineCard 必须含"60m"按钮')
  assert.ok(src.includes('日'), 'MiniKlineCard 必须含"日"按钮')
  assert.ok(src.includes('周'), 'MiniKlineCard 必须含"周"按钮')
  assert.ok(src.includes('月'), 'MiniKlineCard 必须含"月"按钮')
  // 默认日线
  assert.ok(
    src.includes("useState<'1d'") || src.includes("useState<MiniKlineTimeframe>('1d')"),
    'MiniKlineCard 默认周期必须为 1d（日线）',
  )
  // CHANGE-20260714-001: 移除图内 TV 标志（CHANGE-007: 在 controller 中配置）
  assert.ok(
    ctrlSrc.includes('attributionLogo'),
    'miniKlineController 必须设置 layout.attributionLogo=false 移除图内 TV 标志',
  )
  // CHANGE-20260714-001: 删除标题（不得含 mini-kline-title 类）
  assert.ok(
    !src.includes('mini-kline-title'),
    'MiniKlineCard 不得含 mini-kline-title（标题已删除）',
  )
})
...
test('MiniKlineCard 卸载清理：resizeObserver.disconnect + controller.destroy + controllerRef 清空', () => {
  const src = readSource(MINI_KLINE_PATH)
  const ctrlSrc = readSource(CONTROLLER_PATH)
  ...
  // CHANGE-20260715-007: cleanup 通过 controller.destroy() 间接调用 chart.remove()
  assert.ok(src.includes('controller.destroy()'), 'MiniKlineCard 卸载必须调用 controller.destroy()')
  assert.ok(ctrlSrc.includes('chart.remove()'), 'miniKlineController.destroy 必须调用 chart.remove()')
  // ref 必须清空，避免内存泄漏
  assert.ok(
    src.includes('controllerRef.current = null'),
    'MiniKlineCard 卸载必须将 controllerRef 置为 null',
  )
})

test('MiniKlineCard chart 实例仅创建一次（useEffect 空依赖数组，无重复 canvas）', () => {
  const src = readSource(MINI_KLINE_PATH)
  // CHANGE-20260715-007: 创建 controller 的 useEffect 必须使用空依赖数组 []
  assert.ok(
    /useEffect\(\(\)\s*=>\s*\{[\s\S]*?createMiniKlineController[\s\S]*?\},\s*\[\]\)/m.test(src),
    'MiniKlineCard 创建 controller 的 useEffect 必须使用空依赖数组 []（无重复 canvas）',
  )
  // 数据更新 useEffect 依赖 bars + timeframe（timeVisible 按周期切换）
  assert.ok(
    /useEffect\(\(\)\s*=>\s*\{[\s\S]*?setData[\s\S]*?\},\s*\[bars,\s*timeframe\]\)/m.test(src),
    'MiniKlineCard 数据更新 useEffect 必须依赖 [bars, timeframe]（独立于 chart 创建）',
  )
})

test('MiniKlineCard ResizeObserver 响应式宽度（controller.resize）', () => {
  const src = readSource(MINI_KLINE_PATH)
  const ctrlSrc = readSource(CONTROLLER_PATH)
  // 必须使用 ResizeObserver 监听容器宽度
  assert.ok(src.includes('ResizeObserver'), 'MiniKlineCard 必须使用 ResizeObserver')
  // CHANGE-20260715-007: 回调中调用 controller.resize（内部 applyOptions({ width })）
  assert.ok(
    src.includes('controller.resize('),
    'MiniKlineCard ResizeObserver 回调必须调用 controller.resize(width)',
  )
  assert.ok(
    ctrlSrc.includes('applyOptions') && ctrlSrc.includes('width'),
    'miniKlineController.resize 必须调用 chart.applyOptions({ width })',
  )
...
test('useMiniKlineData 只请求活动周期（不预取五周期）', () => {
  const src = readSource(USE_MINI_KLINE_PATH)
  // useBars 调用使用当前 timeframe（不硬编码 1d/1w/1mo）
  assert.ok(
    src.includes('timeframe') && src.includes('BARS_COUNT[timeframe]'),
    'useMiniKlineData 必须按当前活动 timeframe 计算 barsCount（不预取五周期）',
  )
  // 不存在同时调用 useBars 五次的预取逻辑
  const useBarsCallCount = (src.match(/useBars\(/g) || []).length
  assert.ok(
    useBarsCallCount === 1,
    `useMiniKlineData 只允许调用 useBars 一次（实际：${useBarsCallCount} 次），禁止预取五周期`,
  )
})
...
// =================================================================
// 六、stock 列独立筛选 + keyword/preset/URL 同步（CHANGE-20260714-001 契约）
// filterAlias 已移除：stock 列改用普通 FilterPopover（contains/not_contains/eq）；
// 顶部 keyword 独立搜索 symbol/name/pinyin；stock 筛选从 filters 剥离为 stock_name/stock_name_op；
// stock/action 列不入 metric_filters；URL 同步使用 replace 避免循环。
// =================================================================

test('stock 列筛选独立于顶部 keyword（FilterPopover 支持 contains/not_contains/eq）', () => {
  const src = readSource(DATA_TABLE_PATH)
  // 文本列操作符含 contains/not_contains/eq
  assert.ok(
    src.includes('not_contains'),
    'StrategyDataTable 文本列必须支持 not_contains 操作符',
  )
  // 不得使用 <KeywordFilterPopover>（已移除）
  assert.ok(
    !/<KeywordFilterPopover/.test(src),
    'StrategyDataTable 不得使用 <KeywordFilterPopover>（stock 列改用普通 FilterPopover）',
  )
})

test('buildStrategyResultQueryParams 从 filters 剥离 stock 列为 stock_name/stock_name_op', () => {
  const src = readSource(URL_STATE_PATH)
  // 必须存在 stock_name 和 stock_name_op 字段
  assert.ok(
    src.includes('stock_name') && src.includes('stock_name_op'),
    'buildStrategyResultQueryParams 必须输出 stock_name/stock_name_op',
  )
  // 必须存在 extractStockNameFilter 辅助函数（供 Excel 导出复用）
  assert.ok(
    src.includes('extractStockNameFilter'),
    'marketWorkspaceUrlState 必须导出 extractStockNameFilter（Excel 导出复用）',
  )
  // 合法操作符：contains / not_contains / eq
  assert.ok(
    src.includes("'contains'") && src.includes("'not_contains'") && src.includes("'eq'"),
    'stock_name_op 必须支持 contains/not_contains/eq 三种操作符',
  )
})
