...
文字+图片及部分失败语义已确认。Webhook 与平台应用的长期优先级已决定：唯一接入方式为 `feishu_platform_app`，`feishu_webhook` 已永久删除（migration 055 CHECK 约束禁止），不再作为 OPEN 项。仍需决定图片存储方式、用户配置复杂度和真实生产 E2E 验证。
