...
| ALIGN-036 | delivery_worker monitor 资格修复待生产验证 | 代码已实现 `_MONITOR_SOURCE_TYPES` 与 `is_user_eligible_for_monitor` 在 `delivery_worker.py` 投递前复核；测试覆盖 active admin/active member/disabled admin/plain user。生产环境尚未验证真实 monitor_event 能生成 MessageDelivery 并实际投递。 | 部署后检查 monitor_event 来源的 MessageDelivery 为 active admin 与 active member 生成，disabled admin/plain user 被标记 dead/USER_INELIGIBLE | P1 |
| ALIGN-037 | 1d partial daily bar 与 live 1m monitor 待生产验证 | 代码已实现 `MarketDataAggregationService` 交易时段合成 partial daily bar、`monitor_batch_service` 使用 live 1m 输入；测试覆盖交易/非交易场景。生产环境尚未验证真实交易时段 1d bars 返回 `is_partial=true`、monitor cycle 日志输出 `last_minute_bar_time`/`minute_is_partial`。 | 部署后在交易时段 curl `/instruments/{id}/bars?timeframe=1d`，确认最后一根为当日 partial bar；查看 monitor worker 日志包含 `minute_last_bar_time` 与 `minute_is_partial` | P1 |
