...
测试覆盖：
- seed_strategies 在空库上创建 watchlist_monitor 1.3.0 / dsa_selector 1.4.1 released 版本
...
async def test_seed_creates_new_released_versions(db_session):
    """seed_strategies 在空库上创建 1.3.0/1.4.1 released 版本。"""
    ...
    assert ("watchlist_monitor", "1.3.0") in result_map, (
        "watchlist_monitor v1.3.0 必须被 seed_strategies 创建"
    )
    assert ("dsa_selector", "1.4.1") in result_map, (
        "dsa_selector v1.4.1 必须被 seed_strategies 创建"
    )
    assert result_map[("watchlist_monitor", "1.3.0")] == "released"
    assert result_map[("dsa_selector", "1.4.1")] == "released"


@pytest.mark.asyncio
async def test_watchlist_monitor_manifest_contains_new_fields(db_session):
    """watchlist_monitor v1.3.0 manifest outputs 必须包含 previous_close 和 change_pct。"""
    with patch.object(db_session, "commit", new=db_session.flush):
    ...
    stmt = (
        ...
        .where(
            ...
            StrategyVersion.version == "1.3.0",
        )
    )
    result = await db_session.execute(stmt)
    version = result.scalar_one_or_none()
    assert version is not None, "watchlist_monitor v1.3.0 必须存在"
    assert version.status == "released"

    manifest = version.manifest
    output_keys = [o["key"] for o in manifest.get("outputs", [])]
    assert "previous_close" in output_keys, (
        "watchlist_monitor v1.3.0 manifest.outputs 必须包含 previous_close 字段"
    )
    assert "change_pct" in output_keys, (
        "watchlist_monitor v1.3.0 manifest.outputs 必须包含 change_pct 字段"
    )
...
async def test_seed_does_not_modify_old_released_version(db_session):
    ...
    assert new_version is not None, "1.3.0 版本必须仍然存在"
