...
**Factor rebuild 阶段与因子失败门禁（CHANGE-20260717-002；CHANGE-20260721-001 FR-11 缓存失效扩展；CHANGE-20260721-002 V2 四层 + Capture 精确清理）**：盘后顺序为「原始日线刷新（`refreshing_daily`）→ 公司行为/factor 重建（成功）→ 覆盖率门禁/DSA → snapshot 发布」。`AdjustmentFactorService.rebuild_factor_series` 从最早受影响日期完整重建该股票日线 factor 序列并原子 upsert（禁止只更新最近 5 根）；公司行为集合或 fingerprint 变化时触发重建，成功后**精确失效该股票下游四层缓存（FR-11 V2）**：

| 缓存层 | 失效范围 | 失效函数 | 说明 |
|---|---|---|---|
| MDAS | `mdas:{instrument_id}:*` | `_invalidate_mdas_cache`（sync Redis scan+delete） | 行情聚合层缓存，原有失效逻辑保留 |
| bars | `bars:{instrument_id}:*` | `bars_cache.invalidate_bars_cache`（async） | 原始行情响应缓存；`bars_redis_cache_enabled` 默认 False，禁用时返回 0 |
| indicator | `indicator:{instrument_id}:*` | `indicator_cache.invalidate`（async） | 指标计算结果缓存，TTL 300s |
| Capture（V2） | filesystem per-event key（`{event_id}_{instrument_id}_{chart_version}_...`） | `_invalidate_capture_cache`（CHANGE-20260721-002） | 扫描 `CAPTURE_CACHE_DIR` 删除文件名含 `_{instrument_id}_` 的文件；目录不存在/空返回 0；TTL 600s 兜底 |

**FR-11 单层失败不阻塞**：单层缓存失效失败不阻塞其他层（缓存 TTL 会自然过期）；四层失效均不抛异常向上传播，rebuild_factor_series 仍返回成功。
...
| 字段 | 类型 | 语义 |
|---|---|---|
| `trade_date` | `str` (ISO) | 审计业务日期（如 `"2026-07-18"`） |
| `total_audited` | `int` | 审计总股票数 |
| `consistent` | `int` | 因子一致股票数 |
| `needs_rebuild` | `int` | 需重建股票数 |
| `audit_rebuilt` | `int` | 本审计阶段实际重建成功数（区分 `_rebuild_factors_if_needed.rebuilt`） |
| `rebuilt` | `int` | 向后兼容：等于 `audit_rebuilt` |
| `failed` | `int` | 重建失败数 |
| `errors` | `int` | dry_run/rebuild_batch 异常计数 |
| `failed_symbols` | `list[str]` | 失败股票代码列表（便于生产验证定位） |
...
### 2.5.5 三类 IndicatorView 独立飞书图片（CHANGE-20260720-001 §三+§四）

监控事件与详情页发送飞书支持三种独立指标视图，每种视图生成专属图片，禁止混合指标：

- **共享枚举** `backend/app/constants/indicator_view.py`：
  ...
  - `EVENT_TYPE_TO_INDICATOR_VIEW` 自动映射：`bb_*_touch` → `bollinger`，`node_cluster_touch` → `node_cluster`，`smc_*` → `smc`
  - `resolve_indicator_view(event_type, payload)` / `is_valid_indicator_view(value)`
- **三套 Capture Preset** `frontend/src/features/feishu/capturePresets.ts`：
  - `FEISHU_CAPTURE_PRESETS = { node_cluster: {...}, bollinger: {...}, smc: {...} }`
  - 每个 preset 含 `indicator_view` / `timeframe` / `chart_version` / `layers` / `ready_check`
  - **layers 互斥**（除共享 candlestick）：Node=日线 K 线 + 成交量 + Profile + POC + Peak Node + 触发节点；BB=日线 K 线 + BB 上中下轨 + 触发轨道；SMC=日线 K 线 + BOS/CHoCH + 有效 OB + EQH/EQL + Strong/Weak + 触发对象
- **IndicatorView 贯穿全链**：`StrategyEvent.payload` → `NotificationMessage.resource_refs` → Capture 请求 → `CaptureJob.indicator_view`（nullable 兼容历史）→ 输出文件名 → 缓存键 → 幂等键 → 状态查询 → 前端 URL 参数
- **缓存键扩展**：`_build_cache_key` 新增 `iv={indicator_view}` 段；`output_filename` 加 `-{indicator_view}` 后缀
- **文字卡片按 indicator_view 拆分**：`build_monitor_event_text(indicator_view, ...)`：`None` → 全字段；`node_cluster` → 只节点字段；`bollinger` → 只 BB 字段；`smc` → 只 SMC 字段
- **监控自动发送映射**：`monitor_batch_service` 调用 `resolve_indicator_view(first_event.event_type, first_payload)` 自动映射；capture_payload 含 `"indicator_view": indicator_view`
- **详情页 POST body**：`POST /api/v1/instruments/{id}/send-feishu` body schema `SendFeishuRequest`：`indicator_view: Literal["node_cluster", "bollinger", "smc"] | None = None`；`normalized_indicator_view()` 返回默认值或具体值；前端弹窗三单选项（筹码共识价/布林带/SMC 结构）；`useStockDetailFeishu` 新增 `selectedIndicatorView`/`setSelectedIndicatorView`/`handleSendFeishu(indicatorView)`；`stock_detail_feishu_service` 透传到 capture worker，`output_filename` 加 `-{indicator_view}` 后缀，图片消息 `resource_refs` 携带 `indicator_view`

## 2.6 wencai 板块同步（pywencai 唯一数据源）
...
- 配置 `BOARD_SYNC_ENABLED`（CHANGE-20260716-007：环境变量注入，默认 `false`；PR #77 收口第二轮：严格解析）：`config.py` 的 `_resolve_board_sync_enabled()` 优先级为「环境变量 > CONFIG_FILE > 默认 False」；`docker-compose.prod.yml` 的 worker-after-close 服务显式注入 `BOARD_SYNC_ENABLED: ${BOARD_SYNC_ENABLED:-false}`；**PR #77 收口第二轮严格解析**：truthy 集合 `{1, "1", true, "true", "yes", "on"}`（大小写不敏感）/ falsy 集合 `{0, "0", false, "false", "no", "off", ""}`；非法值（如 `"maybe"`）抛 `RuntimeError` fail-fast；同时处理 config 文件中 bool 与 str 两种类型；修复旧实现 `bool("false")` 恒为 True 的 bug；当 `false` 时 `syncing_boards` 步骤直接跳过执行，不发起任何 pywencai 请求，记录 `scheduler_job_runs.status='skipped'` + `reason_code='board_sync_disabled'`；当 `true` 时按流程执行同步；
- **Dockerfile Node.js 依赖**（CHANGE-20260716-007）：`backend/Dockerfile` 安装 `nodejs`，pywencai 的 `get_token()` 通过 `subprocess.run(['node', ...])` 执行 `hexin-v.bundle.js` 计算反爬 token；容器内必须存在 `node` 二进制；
- **source=wencai 修复**（CHANGE-20260716-007）：`board_sync_service.sync_boards()` 成功返回的 dict 显式带 `source: "wencai"`，防止手工调用 `record_sync_status(result)` 时丢失 source；`/market/boards` 不再返回 `source=None`；
- **事件 + metadata 持久化（PR #77 收口第二轮）**：`after_close_orchestrator.py::_record_board_sync_outcome()` 在 success/failure/skip 三分支同时写入 `job_run_events`（通过 `append_event`）和 `SchedulerJobRun.metadata_json.board_sync_result` 字段；success 写 info 级（含 source/raw_rows/resolved/unresolved/industry_count/concept_count/membership_count/duration_ms），failure 写 warn 级（含 error_code/reused_previous_snapshot），skip 写 info 级（含 reason_code）；保证 Redis 缺失时仍可从 job metadata 回退读取同步状态；
- **ALIGN-041（RESOLVED）**：pywencai 配合 `WENCAI_COOKIE` 在物理机真实烟测返回完整目录与代表性成分，ALIGN-041 已关闭；
- **单一 provider**：`sync_boards` 通过 `WencaiBoardProvider` 调用 pywencai，查询 `同花顺概念，行业分类`；禁止 provider 混用或 fallback 链；pywencai 不可用时直接软失败，保留上一成功版本；
- **HTTP 重试**：provider 内置 3 次重试（带 Referer 头），禁止在 service 层再包一层重试导致重试放大；
- **总超时**：`SYNC_TOTAL_TIMEOUT_SECONDS=1800`（30 分钟）覆盖整个同步流程（拉取目录 + 成分 + 校验 + 原子切换），超时即标记 `failed` 并保留上一成功版本。

### 2.6.1 同步流程

```text
board_sync_service.sync_boards()
  → WencaiBoardProvider.fetch_boards() 拉取行业 + 概念板块目录（pywencai 查询，asyncio.to_thread 包装）
  → WencaiBoardProvider.fetch_memberships() 逐板块拉取成分股代码
  → 暂存到 BoardSnapshot
  → 完整性门禁校验（绝对门禁 + 相对门禁）
  → 事务内原子切换（TRUNCATE + INSERT）
  → caller 显式 commit
```

- `WencaiBoardProvider` 通过 `asyncio.to_thread` 包装 pywencai 同步调用，不阻塞事件循环；
- pywencai 只存在于 provider 模块，用户请求链不直接调用 pywencai；
- `BoardSnapshot` 暂存结构与门禁校验是纯函数，可直接单元测试。

### 2.6.2 完整性门禁

校验项分为绝对门禁（任一失败即拒绝切换）和相对门禁（与上一成功版本比较）：

**绝对门禁**（任一不满足即拒绝）：

| 校验项 | 阈值 | 说明 |
|---|---|---|
| 原始记录数 | `raw_count >= 5000` | pywencai 返回的原始记录总数下限 |
| 代码唯一性 | `uniqueness >= 99.9%` | 成分股代码去重后的唯一性比例 |
| 行业板块数 | `industry_count >= 200` | 行业板块目录下限 |
| 概念板块数 | `concept_count >= 300` | 概念板块目录下限 |
| 关系数 | `membership_count >= 60000` | 板块-成分股关系总数下限 |
| 解析率 | `parse_rate >= 95%` | pywencai 返回结果的可解析比例 |

**相对门禁**（与上一成功版本比较）：

| 校验项 | 阈值 | 说明 |
|---|---|---|
| 异常降幅 | `MAX_DROP_PERCENT = 0.20` | 板块数/成分数较上次降幅 >20% 拒绝；首次同步（prev=0）不做降幅检查 |

### 2.6.3 事务边界与失败处理

- **失败保持上一成功版本**：校验失败或异常时不删除旧关系，旧数据保留可用，前端 `stale=true` 标识；
- `BoardSnapshot` 原子切换在单个事务内执行（`delete MarketBoardMembership` → `delete MarketBoard` → `insert` 新数据），caller 显式 commit；
- `sync_boards` 不内部 commit，由调用方控制事务；
- 校验失败返回 `status='validation_failed'`，异常返回 `status='failed'`，成功返回 `status='succeeded'`；
- **软失败编排**：`syncing_boards` 步骤失败不阻断 DSA/snapshot/publish 链路，after_close_orchestrator 继续执行后续步骤。

### 2.6.4 数据范围与并发

- **只保存最新关系**：不增加历史日期维度，不存板块历史/行情/资金流；
- 单并发执行，批量 500-1000（`BATCH_SIZE = 500`）；
- pywencai 拉取设置连接/读取超时与 3 次有限重试；
- 失败重试后继续沿用上次成功快照并告警。
...
- `syncing_boards` 失败/跳过保留旧板块数据，不影响后续 DSA/snapshot/publish 步骤；
- board_sync 不是独立 `WORKER_TYPE`，不新增 Docker 服务，复用 `worker-after-close` 容器。

### 2.6.6 Redis 状态跟踪与 DB 回退

- `record_sync_status()` / `get_sync_status()` 写入 Redis key `board_sync:status`，TTL 7 天；
- 记录字段含 `last_attempt_status`（最近一次同步状态）、`last_success_at`（最近一次成功时间）、`source`（数据源标识）；
- `/market/boards` 响应读取该状态生成 `source`/`stale`/`last_attempt_status` 字段；
- `stale=true` 当且仅当：旧数据存在 + 最近一次同步失败/校验失败；
- **Redis → job metadata 回退（PR #77 收口第二轮）**：`market_stocks_service._get_board_sync_status_from_job()` 查询近期 `SchedulerJobRun.metadata_json.board_sync_result`；`get_market_boards()` 在 Redis `get_sync_status()` 返回 None 时回退到 job metadata；job metadata 由 `_record_board_sync_outcome()` 写入（详见 §2.6.0）；
- **`source="unknown"` 语义（PR #77 收口第二轮）**：当 DB 有数据但 Redis 与 job metadata 均无状态源时，`source="unknown"`（非 None），避免前端把「未知」误判为「从未同步」；前端应将 `unknown` 显示为「数据源未知」提示，而非禁用筛选输入。
