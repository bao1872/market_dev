...
#
# generate_latest 不在 try 块中导入，而是作为 _prometheus_generate_latest 单独导入，
# 避免与模块级 generate_latest 包装函数形成条件函数重定义（mypy [misc] 错误）。
# ---------------------------------------------------------------------------
...
# 回退注册表：仅在 fallback 模式下填充 _FallbackMetric 实例
...
if not _PROMETHEUS_AVAILABLE:
    for metric in (
        ...
        if isinstance(metric, _FallbackMetric):
...
def generate_latest() -> bytes:
    """返回 Prometheus exposition 格式文本。

    优先委托 prometheus_client.generate_latest；prometheus_client 缺失时
    使用回退实现序列化 _REGISTRY 中的指标。
    """
    if _prometheus_generate_latest is not None:
        return _prometheus_generate_latest()
    # 回退实现：遍历 _REGISTRY 序列化为 Prometheus 文本
    lines: list[str] = []
