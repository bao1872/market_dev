...
| ALIGN-040 | 统一行情工作区代码未部署/生产未验收 | 当前修复分支 `fix/pr74-context-layers-remove-consensus` 已重构 `/market`：移除 `MarketInstrumentPane`/`StockResearchWorkspace`/`ResearchContextPanel`，改为 `MarketToolbar` + `MarketStockTable`（服务端分页）+ `EventStatePanel`（可收起，首次默认收起）；`/market` 明确禁止挂载 K 线。`/stock/:symbol` 仍复用 `StockResearchWorkspace`（K 线研究区）。`/admin/stock-debug` 独立管理员调试路由保留。P0-5: `StrategyChart` 的 `tv-strategy-legend` 只读行已删除，图层开关唯一真源在 `IndicatorToolbar`。P0-3: `market_stocks_service` 已删除 `latest_event` 批量查询（8 条固定 SQL）。P0-6: MACD 默认关闭。前端 node tests/tsc/eslint/vite build 全通过。已部署 SHA `c92b8e8`，20 项浏览器 E2E 全 PASS。但代码尚未合并到 main。 | 部署后：1) 生产 `/market` 验证无 K 线 + 表格默认列 + 右栏默认收起 + boardsAvailable 单一真源；2) `/admin/stock-debug/:symbol` 验证管理员可见原始因子/JSON、普通用户 403；3) `/stock/:symbol` 复用共享图表区 + 单一工具栏（无 tv-strategy-legend）+ MACD 默认关闭；4) returnTo 外部 URL 被拒绝 | P1 |
| ALIGN-042 | ScreenerPage/MessagesPage 导航 URL 仍用旧契约 | `ScreenerPage` 的 `buildMarketEntryFromScreener` 仍生成 `/market?scope=market&symbol=...&source=selection&strategy=dsa_selector&returnTo=...`（旧契约），`MessagesPage` 仍生成 `/market?symbol=...&event_id=...`。但 `/market` 的 `MarketWorkspacePage` 只读取 `scope/query/page/page_size/sort/selected/industry/concept/state` 新契约，不读取 `symbol/source/strategy`。`event_id` 仍在 `marketWorkspaceUrlState.ts` 代码中（过渡态）。文档已更新为目标契约（ScreenerPage → `/stock/:symbol`，MessagesPage → `/stock/:symbol?event_id=...`），但代码尚未对齐。 | 修复 `buildMarketEntryFromScreener`/`buildMarketEntryFromMessage` 导航到 `/stock/:symbol`；清理 `marketWorkspaceUrlState.ts` 中的 `eventId`/`event_id` 过渡代码 | P2 |
