...
| market_data | `api/bars.py`, `api/market.py`, `api/calendar.py`, `api/instruments.py` | `market_data_aggregation_service.py`, `calendar_seed.py` | bar/instrument/calendar models & repositories | 页面、指标、截图必须同源 |
| screening | `api/strategies.py`, `api/strategy_runs.py` | `strategy_batch_service.py`, strategy runtime | `strategy_*` models | 发布门禁关键模块 |
| watchlist | `api/watchlist.py` | watchlist service / limit logic | `user_watchlist_items` model | 到期权限和额度检查 |
...
| notifications | `api/notifications.py`, `api/stock_detail_feishu.py` | `outbox_relay.py`, `delivery_worker.py`, `stock_detail_feishu_service.py`, `channel_adapter.py`, `feishu_card_builder.py` | notification/outbox/delivery models | 飞书、图文、重试 |
