...
| 盘后调度入口 Phase 8A（16:00 创建 after-close run 幂等 / 18:30 兜底幂等 / `_maybe_trigger_after_close_orchestrator` 废弃为 no-op / DSA 所有权 `_owner` 排除 / manual DSA 仍可领取 / `trigger_dsa=False` / `publish_run` 幂等崩溃恢复 / watchlist_ready 仅 published+full+schema v5 / legacy 状态映射 / compose 同镜像变量） | `backend/tests/test_phase8a_e2e_correctness.py`（10 个测试）；`backend/tests/test_worker_auto_trigger.py`（4 个旧测试，验证 no-op 不触发） |
...
| `sourceListVisibilityContract.test.ts` | 31 tests | 详情页来源列表可见性合同（CHANGE-20260725-003）：missing_origin invalid 占位（缺 originScope + 无有效 /market returnTo → origin=watchlist 占位 + contextInvalid + invalidReason="missing_origin"）、direct 边界（显式 originScope=direct 唯一触发单列无左栏，其他场景禁止静默单列）、入口矩阵（market/watchlist 入口必须携带完整 sourceRunId + canonicalQuery，缺任一字段视为 missing_origin invalid）、invalid 占位渲染 `data-invalid-reason` 属性 + `INVALID_REASON_LABELS` 中文文案（禁止空占位）、防闪烁（与 sourceListStabilityContract 互补）、V2 唯一真源（V1 resolveDetailSourceContext/resolveStockDetailOrigin 已 @deprecated，禁止新增 V1 调用） |
