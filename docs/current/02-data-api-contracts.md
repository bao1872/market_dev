...
`MarketDataAggregationService` (MDAS) 是行情读取唯一出口（详见 `docs/changes/records/CHANGE-20260717-002.md`）。`/bars` API 与所有业务调用方（indicator/SMC、strategy_batch、feature_snapshot、chart_bars、structural、capture、monitor）均经 MDAS。

**请求参数**（`/bars` 透传 + 业务调用方显式传）：

| 参数 | 默认 | 语义 |
|---|---|---|
| `timeframe` | — | `1d`/`15m`/`1h`/`1m`/`1w`/`1mo` |
| `adj` | `qfq` | `none`/`qfq`；`none` 时 `adj_factor_hash` 为空 |
| `include_realtime` | False | 是否包含 partial/实时 bar |
| `completed_only` | False | 仅返回已完成 bar |
| `start_date`/`end_date` | None | 时间窗口 |
| `limit` | None | bar 数上限 |
| `warmup_bars` | None | 额外 warmup（SMC 等） |
| `adjustment_as_of` | None | point-in-time 复权锚点（None=最新因子，向后兼容） |

**返回诊断字段**（`BarListResponse`）：

| 字段 | 语义 |
|---|---|
| `market_data_contract_version` | 契约版本（隔离旧客户端缓存） |
| `source_bar_hash` | raw OHLCV + 时间 SHA256[:16]（跨调用方一致性校验） |
...
**复权契约**：
- 公式 `qfq_price = raw_price × factor(bar_date) / factor(as_of)`；`factor(as_of)` 取截断因子序列（`trade_date <= as_of`）最后一个因子（ffill），禁止未来除权事件泄漏。
- raw bar 保持不复权；qfq 只在 MDAS 出口应用一次，不信任 bar 自带 `adj_factor` 列（pytdx hybrid / 15m/60m/1m 行内旧值可能为 1.0）。
- 日内（15m/60m/1m）同一交易日映射同一权威日线因子；周/月"日线完成复权后聚合"。
- 缓存键含全部参数 + contract_version + algorithm_version，true/false 状态隔离。

**跨调用方一致性**：同一股票/周期/截止日下，`/bars`、indicator/SMC、strategy_batch、feature_snapshot 的时间序列、OHLC、`source_bar_hash`、`adj_factor_hash` 必须一致。

**quote 边界**：`/quote` 允许独立实时报价出口，但**不得作为历史 K 线或指标输入**；实时价格只由 quote overlay 呈现，禁止后端 partial bar 与 quote 重复合并。

### 1.2 Indicators API V2 参数与 DisplayWindowSpec（CHANGE-20260721-002）

`/api/v1/instruments/{instrument_id}/indicators` 新增与 `/bars` 同款的 V2 参数，确保指标计算与 bars 请求使用同一份行情口径：

| 参数 | 默认 | 语义 |
|---|---|---|
| `timeframe` | — | `1d`/`15m`/`1h`/`1m`/`1w`/`1mo` |
| `adj` | `qfq` | `none`/`qfq` |
| `include_realtime` | False | 是否包含 partial/实时 bar（与 `/bars` 同款） |
| `completed_only` | False | 仅使用已完成 bar 计算指标（与 `/bars` 同款） |
| `adjustment_as_of` | None | point-in-time 复权锚点（与 `/bars` 同款） |
| `include_smc` | False | 是否包含 SMC 图层（CHANGE-011，原参数保留） |

**`DisplayWindowSpec` dataclass**（`backend/app/services/indicator_display_frame.py`）封装三参数（`include_realtime`/`completed_only`/`adjustment_as_of`），`to_cache_suffix()` 方法生成缓存后缀（如 `rt=False_co=True_ao=2026-07-18`），追加到 indicator_cache key 末尾，不同 spec 独立缓存。

**`display_frame` V2 字段**（`BarListResponse.display_frame` / indicators API 响应 `display_frame`）：

| 字段 | 语义 |
|---|---|
| `requested_count` | 请求的 bar 数（来自 `INDICATOR_BARS[timeframe]` 或 `limit` 参数） |
| `actual_count` | 实际返回的 bar 数（可能因数据不足小于 `requested_count`） |
| `first_time` / `last_time` | 首末 bar 时间戳（ISO 字符串） |
| `include_realtime` | 是否包含 partial/实时 bar（回显） |
| `is_partial` | 末根 bar 是否为 partial |
| `adjustment_as_of` | 回权锚点（回显） |
| `source_bar_hash` | raw OHLCV + 时间 SHA256[:16]（V1 保留） |
| `adj_factor_hash` | 因子序列 SHA256[:16]（V1 保留，adj=none 时为空） |

**`ALGORITHM_VERSION` v11→v12**（CHANGE-20260721-002）：删除 `_display_window=100` 硬编码，改用请求 bars 参数；旧 v11 缓存的 display_frame 缺失 V2 字段且展示窗口固定 100 根，必须强制失效；`build_cache_key` 新增 `spec_suffix` 参数（DisplayWindowSpec 后缀）。
...
| 行情 | `/instruments`, `/calendar`, `/market`, `/bars` | 数据新鲜度、partial/degraded 标识；`/instruments/{id}/bars` page_size 按 timeframe 限制：`15m` 最大 4000，`1h` 最大 1200，其他最大 1000；`/instruments/{id}/indicators` 的 `bars` 参数最大 4000；`indicators` 响应含 `sqzmom_lb` 全局技术指标数据，后端逐行复刻 Pine 代码，前端只渲染不计算；**SMC 指标按需启用**（`include_smc` 查询参数，默认 `false`，`true` 时返回 `data.smc` 且 cache key 追加 `:smc` 后缀，详见第 9.5 节）；FVG 完全排除；**`GET /market/stocks` 行业/概念筛选**（PRD §7.5 pywencai 同步后；CHANGE-20260716-007：industry 改关键词匹配；PR #77 收口第二轮：concept 规范化精确匹配）：`industry` 参数为关键词（`MarketBoard.name.ilike('%keyword%', escape='\\')`，匹配完整路径任意一级；`_normalize_keyword` NFKC + trim，`_escape_ilike_pattern` 转义 `\`/`%`/`_`），`concept` 参数为精确匹配（PR #77 收口第二轮：concept 也应用 `_normalize_keyword()` NFKC + trim 后再 `==` 比较，与 industry 共用同一规范化函数，避免全角/半角不一致）；通过共享 `backend/app/repositories/board_filter_helper.py::build_board_filter_conditions` 构造 EXISTS 子查询查询 `market_boards` 表实现；未同步板块数据时返回空列表（不报 422）；`industry` + `concept` 同时传时取交集（AND 语义）；行业值 `-` 在 API 层不变（前端可渲染为 `/`）；market stocks/StrategyRunResults/行情/自选/Excel 复用同一 helper；**`GET /market/boards?type=industry\|concept`** 返回 `MarketBoardsResponse`：`items`（板块目录列表）+ `available: bool`（items 非空时 true）+ `reason_code: str \| null`（无数据时为 `"board_provider_unavailable"`）+ `updated_at` + `source: str \| null`（数据源标识，如 `"wencai"`；PR #77 收口第二轮：DB 有数据但 Redis 与 job metadata 均无状态源时为 `"unknown"`，非 null，避免前端误判为「从未同步」）+ `stale: bool`（存在旧数据但最新同步失败时为 `true`，前端展示"沿用上次板块数据"）+ `last_attempt_status: str \| null`（最近一次同步状态：`succeeded`/`failed`/`validation_failed`/`skipped`；PR #77 收口第二轮：Redis 缺失时从 `SchedulerJobRun.metadata_json.board_sync_result` 回退）；前端依据 `available` 决定筛选输入是否禁用，依据 `stale` 决定是否展示"沿用上次板块数据"提示 |
...
| `industry` | string \| null | max_length=100 | 行业关键词（CHANGE-20260713-006，CHANGE-20260716-007：完整路径存储、ilike 匹配任意层级；JSONB 列无新 migration） |
| `concept` | string \| null | max_length=100 | 概念板块名称（精确匹配；CHANGE-20260713-006，JSONB 列无新 migration） |
...
- 普通用户响应：`contractVersion / meta / asOf / core / auxiliary / availability / recentChanges / dataQuality`；**CHANGE-20260716-006**：新增顶层 `productObservations`（产品观察扩展，独立于冻结 Core 14）+ `recentChanges` 新增 `latestChangesFrom`/`latestChangesAsOf` 字段（仅比较最新交易日与前一发布交易日，limit 10→2，删除 30 条上限）；`meta` 为 `AtomicFactsMeta`（`payloadVersion`/`researchFreezeVersion`/`presentationVersion`）；用户 DTO `PublicAtomicFactItem`（`publicKey / dimension / label / visualKind / value / valueText / categoryCode / categoryLabel / secondaryText / unit / thresholdEnabled`），**不含 factId / sourcePath / formula / thresholdRef**（普通用户接口完全不泄露内部字段）；**产品观察 DTO** `ProductObservationItem`（`publicKey / scope / label / visualKind / value / valueText / categoryLabel / secondaryText`），`scope="product"` 与 Core/Auxiliary 区分；
...
- **双合同分离**：冻结研究合同 `atomic_fact_contract_v1.json`（V4.13 原字段，14 Core / 10 Aux / 1 Rejected，**不含 public_key/public_label 等产品层语义**）+ 产品展示合同 `atomic_fact_presentation_v1.json`（按 Fact ID 映射 `publicKey/publicLabel/visualKind/valuePrecision/groupTitle/secondaryLabel`，**恰好 14 Core + 8 Aux，排除 T3/T6/V1**）；生产服务同时读取两份合同（frozen 决定事实/顺序/公式/阈值/路径，presentation 决定产品文案与 UI 类型）；presentation 顶层新增 `unclassifiedLabel` 字段，分类未启用时使用。
- **产品观察扩展（CHANGE-20260716-006）**：独立第三合同 `atomic_fact_product_observations_v1.json`（sourceFields 仅 `raw`/`confirmed_high`/`confirmed_low`），**不修改冻结 Core 14**、不修改 `atomic_fact_contract_v1.json`、不修改 `atomic_fact_presentation_v1.json`、不修改 `structural_factor_service.py` worker 持久化链；`compute_product_observations` 从已存在 `structural_payload.primary.1d` 字段（`price_position_in_confirmed_swing_raw`）派生 `inside`（`inside = raw if 0<=raw<=1 else None`），区间外数值不静默 clip；首个观察项 `confirmed_swing_position`（`publicKey=confirmed_swing_position`，`visualKind=confirmed_position`，`scope=product`）表示"价格在最近确认区间的位置"，0.33/0.67 边界与 S3 一致；<0 显示"低于确认区间"，>1 显示"高于确认区间"；产品观察**不计入 Core 14 / availability.coreDenominator**，仅作为顶层 `productObservations` 字段返回。
...
- **presentation `secondaryLabel` 为单位/弱说明的唯一真源**：所有 secondaryText 由 presentation 合同映射生成（`_secondary_text_for` 辅助函数），分类未启用时使用 `unclassifiedLabel`；服务中禁止散落 `ATR / 根日K`、`个交易日`、`分类未启用` 等重复常量；
- **persisted-first**：Context API 优先读取已持久化 `summary_payload.atomic_fact_contract_v1`，由 `PersistedAtomicFactsPayload` Pydantic schema 严格校验（`extra="forbid"` + `model_validator`）：四版本完全匹配、core 键恰好 `trend/momentum/structure/volume`、每一项通过 `PublicAtomicFactItem`、publicKey 属于正确维度且无重复/未知、T3/T6/V1 不存在、availability 与实际数组及固定分母 14 一致、不含 debug；任一不满足 → 同一纯函数 fallback 重算（**不回写旧快照**，不返回 500）；
- **M5 任一缺失即缺失**：`_squeeze_state` 改为 `if on is None or off is None: return None`（任一输入缺失即缺失，旧 `and` 改 `or`）；
- `as_of` 语义为**截止日期**：查 `trade_date <= as_of`，按 `trade_date`/`published_at`/`finished_at` 倒序取最新 1 条；周末和无批次日期必须返回之前最近发布状态（禁止返回未来快照或未来变化）；`recent_changes` 按 `as_of` 过滤 ≤ 该日期的已发布快照；
...
- 无 run → 空态（`core` 分母仍固定 14，全部缺失，`reasonCode=no_published_full_run`）；无快照 → `reasonCode=snapshot_missing`；
- GET 请求零数据库写入（仅 `SELECT`）。

### 15.2 事实计数与展示规则

- Core 严格 **14**（趋势4 + 动量4 + 结构5 + 成交1），分母 `availability.coreDenominator` 固定 14；S2 存在；
- 缺失事实由 `compute_atomic_facts` **从 Core 数组直接省略**（非灰显伪装），`availability.coreMissing` 用 publicKey 列出；展示用 Auxiliary 为 8 项（排除 T3/T6/V1），其余 2 项 Aux 默认隐藏（不在普通用户 UI 展示），`availability.auxiliaryHidden` 为响应中实际返回的 Auxiliary ID；**10 个 Aux 中仅 8 个可展开**（T3/T6/V1 永不出现 DOM）；
...
- `compute_recent_changes` 接收 ≤10 个升序已发布快照，对 14 个 Core 事实逐对对比 `category`/`value` 变化，最多 30 条；**CHANGE-20260716-006**：`_find_recent_published_snapshots` limit 10→2，只比较最新交易日与前一发布交易日（不再展示多日历史变化），同时删除 `compute_recent_changes` 30 条上限；
- 响应新增 `latestChangesFrom`（前一发布交易日）/`latestChangesAsOf`（最新发布交易日）顶层字段，用于前端空态文案"最近交易日（{asOf}）无状态变化"；
- 每条含中文 `label`（来自 presentation `publicLabel`，非 publicKey）、`publicKey`/`dimension`/`fromText`/`toText`/`deltaText`（变化类型文案）/`asOf` 日期；
- **按每个 Fact 的 presentation `valuePrecision` 量化**（`_quantize_fact_value`，禁止统一 `round(...,4)`）：T2/M2=4 位、M3=6 位、S3/S7/S8/T5/V3=2 位，整数=0 位；
- **事实消失时 dimension 正确**：`FACT_DIMENSION_BY_ID`（从冻结合同导出的 `fact_id → dimension` 映射）确保事实从存在变缺失时仍返回正确维度（禁止默认 trend）；
- `from`/`to` 通过 `_combine_text` 组合短值和 category，避免丢失 M3 双文本状态；relation 类 fromText/toText 回退 categoryLabel；
...
| `industry` | string \| null | 否 | 行业关键词（ilike 匹配完整路径任意层级；CHANGE-20260716-007） |
| `concept` | string \| null | 否 | 概念板块名称（精确匹配） |
