...
# ---------------------------------------------------------------------------
# §7 Board event isolation
# ---------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_board_event_query_only_counts_v2():
    """§7 真实 PG：Board 事件计数只统计 review-history-v2，legacy NULL 不计。"""
    from sqlalchemy import delete, func, select
    ...
    async with AsyncSessionLocal() as s:
        ...
        s.add_all([
            FirstPyramidHistoryEvent(
                instrument_id=iid, algorithm_version=algo, event_type="BOS",
                event_id="b1", history_contract_version=None,
                event_payload={"type": "BOS"},
            ),
            FirstPyramidHistoryEvent(
                instrument_id=iid, algorithm_version=algo, event_type="BOS",
                event_id="b1", history_contract_version=HISTORY_CONTRACT_VERSION,
                event_payload={"type": "BOS"},
            ),
        ])
        await s.commit()

        # 直接验证 Board 计数逻辑：只数 v2
        # （此处验证底层查询语义与 Board filter 一致）
        v2_bos = (await s.execute(
            select(func.count()).select_from(FirstPyramidHistoryEvent).where(
                FirstPyramidHistoryEvent.instrument_id == iid,
                FirstPyramidHistoryEvent.event_type == "BOS",
                FirstPyramidHistoryEvent.history_contract_version == HISTORY_CONTRACT_VERSION,
            )
        )).scalar()
        legacy_bos = (await s.execute(
            select(func.count()).select_from(FirstPyramidHistoryEvent).where(
                FirstPyramidHistoryEvent.instrument_id == iid,
                FirstPyramidHistoryEvent.event_type == "BOS",
                FirstPyramidHistoryEvent.history_contract_version.is_(None),
            )
        )).scalar()
        assert v2_bos == 1
        assert legacy_bos == 1
