...
|---|---|---|---|
| worker-bars-scheduler | bars_scheduler | bars*, strategy_runs, scheduler_job_runs, market_boards, market_board_memberships | 行情覆盖不足影响 DSA；复用 `BarsCoverageService` 统一口径，业务日期使用 `shanghai_business_date()`；**board_sync job 注册在此 worker 内**（17:00 CronTrigger，max_instances=1，与 bars_refresh 16:00 共用同一 AsyncIOScheduler） |
...
| **qstock board sync（已接入 bars_scheduler）** | -（无独立 compose 服务，复用 `worker-bars-scheduler`） | `market_boards`, `market_board_memberships` | **board_sync 作为 job 注册在 `run_bars_scheduler_worker()` 内的 `AsyncIOScheduler`**，每日 17:00（Asia/Shanghai）`CronTrigger` 触发，`max_instances=1` 单并发；**配置 `BOARD_SYNC_ENABLED`（默认 `false`）**：`false` 时 `scheduled_board_sync` 跳过执行，不发任何 THS 请求，记录 `scheduler_job_runs.status='skipped'` + `reason_code='board_provider_unavailable'`；`true` 时按原流程执行；**总超时 `SYNC_TOTAL_TIMEOUT_SECONDS=1800`**（30 分钟）覆盖拉取+校验+切换全流程；`board_sync_scheduler` **不是**有效 `WORKER_TYPE`，`worker-board-sync` Docker 服务已移除，`job_name="board_sync_scheduler"` 仅用于 `SchedulerJobRun` 跟踪；服务入口 `board_sync_service.sync_boards()`；拉取适配器 `qstock_fetcher.QStockFetcher`（延迟导入 qstock，qstock 同步调用通过 `asyncio.to_thread()` 包装，不阻塞事件循环）；流程：拉取板块目录 + 成分 → 暂存集合 → 完整性校验（空集合/目录数≥100/成分数≥3000/异常降幅≤20%）→ 事务原子切换（TRUNCATE+INSERT）；**失败保持上一成功版本，不删除旧关系**；批量 500-1000；只保存最新关系，不存板块历史/行情/资金流；详见 `current/03-jobs-integrations-operations.md` qstock 板块同步章节 |
