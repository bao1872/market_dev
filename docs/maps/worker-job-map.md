...
| scripts.research_feature_matrix_backfill | - | -（不写 DB，骨架阶段不写文件） | 研究特征矩阵 CLI 骨架；与生产 `feature_snapshot_backfill` 严格分离：不接入 `watchlist_ready`，不修改 production snapshot，不新增数据库表；字段因果口径由 `app/research/feature_causality_registry.py` 统一登记（4 命名空间 `causal`/`confirmed_delay`/`hindsight`/`label`）；默认 `--dry-run`，无 `--output` 不落盘；`--output` 必须配合 sample scope（`--symbols` 或 `--limit-instruments`），`_validate_output_scope` 校验失败抛 `ValueError`，禁止无过滤全市场输出文件；`hindsight.*`/`label.*` 禁止进入回测；本 PR 只完成骨架 + dry-run，不实现实际计算；详见 `current/06-research-feature-matrix.md` |
