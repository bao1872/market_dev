...
> 验证日期：2026-07-03  
> 验证状态：本地验证完成；`verified_implementation_commit` 的 GitHub Actions CI 验证完成（Run #39，最终 HEAD `4de9811` 全部 blocking jobs success）；`release/docs-aligned-candidate-v3` 最终 HEAD `aab2c72` 的 GitHub Actions CI 验证完成（Run #42，全部 blocking jobs success；非阻断的 Ruff/Mypy Full Repository Report 失败为预期历史债务展示）；生产未验证  
...
本表是代码基线 `ac07d4b439d45fda1ac5da78afcb471771c67934`（release_merge_commit）的实现事实快照，只记录“当前确认设计已经明确，但实现、测试、部署或生产表现尚未一致”的问题。未决产品问题进入 `17-open-decisions.md`；历史经过进入 CHANGE。

只要本表仍有未关闭条目，就只能声明“文档已准确对齐当前代码及其差异”，不能声明“代码已经完全符合设计”。

已知差异均登记于 Alignment，生产 E2E 和历史质量债务仍为 KNOWN_GAP。

| ID | 领域 | 当前证据与差异 | 目标设计 | 状态 | 关闭证据 |
|---|---|---|---|---|---|
| ALIGN-001 | Git 分支 | release 候选 `release/docs-aligned-candidate-v3` 已从 `origin/main` 建立并通过 `--no-ff` merge 合并 `fix/release-remaining-alignment-gaps`；PR 尚未合入 `main` | 从修复分支建立精确 release 候选，最终通过 PR merge commit 合入 main | `KNOWN_GAP` | release candidate 已建立，PR 未合入 main；merge commit `ac07d4b` 第一 parent 为 `origin/main`（`69ea24e`），第二 parent 为 `origin/fix/release-remaining-alignment-gaps`（`4de9811`）；见 CHANGE-20260703-012 |
...
| ALIGN-010 | 飞书图片 | 后端已实现独立 card/image 状态、partial_failed、failed_step/error_code/error_message 三字段、仅重试图片、502 响应体解析；真实渠道生产 E2E 尚未执行 | 独立 card/image 状态、partial_failed、仅重试图片、真实 E2E | `KNOWN_GAP` | Phase C 已实现状态机（`stock_detail_feishu_service` 返回 partial_failed + failed_step/error_code/error_message，`capture_resp.raise_for_status()` 解析 502 响应体）；`tests/test_state_machine.py` 通过、`tests/test_stock_detail_feishu_status.py` 通过；真实渠道生产 E2E 待 Phase F 部署后执行 |
...
| ALIGN-017 | 飞书渠道 | Phase C 已永久删除 `FeishuWebhookAdapter`，统一为 `feishu_platform_app`；migration 055 添加 CHECK 约束禁止 `feishu_webhook`；hotfix 删除 `system_channel.py`，管理员通知复用管理员用户自己的 Platform App 渠道 | Platform App only，删除全部 Webhook，管理员通知不维护独立凭证 | `CLOSED` | `backend/app/services/feishu_webhook_adapter.py` 已删除；`backend/app/constants/system_channel.py` 已删除；`notification_service.py` / `outbox_relay.py` / `beta_application_notifier.py` / `channel_adapter.py` 改用 `FeishuPlatformAppAdapter`；migration 055 添加 CHECK 约束；管理员通知通过 `beta_application.admin_notification.created` 专用 Outbox 事件扩张，查询 active admin 用户的 active `feishu_platform_app` 渠道；`tests/test_feishu_platform_app_only.py` + `tests/test_beta_application_notifier.py` 全部通过；CHANGE-20260702-009 + CHANGE-20260703-013 |
| ALIGN-018 | Capture 链路 | Phase C 已实现专用 `/capture/stock/:symbol` 前端路由 + `/api/v1/capture/stocks/{instrument_id}/snapshot` 后端 API + Capture Token 隔离；`CaptureStockPage.tsx` 已重构为仅调用 Snapshot API，`stock_capture_service.py` URL 已携带 `instrument_id`，状态机字段已补齐 | 专用 `/capture/stock/:symbol` 路由 + Capture Token 校验 + Snapshot API 唯一数据请求 + URL 携带 `instrument_id` + 完整状态机字段 | `CLOSED` | CHANGE-20260702-011 已重构 `CaptureStockPage.tsx` 仅调用 Snapshot API、`stock_capture_service.py` URL 携带 `instrument_id`、补齐 `capture_status`/`image_upload_status`/`overall_status` 状态机；`tests/test_capture_snapshot.py`、`tests/test_capture_token_isolation.py`、`tests/test_state_machine.py` 通过；frontend contract 52 passed；真实飞书生产 E2E 待 Phase F 部署后执行 |
...
| ALIGN-023 | Worker 心跳僵尸 | `_recovery_watchdog_loop` 只调用 `recover_stale_scheduler_job_runs` 清理 `scheduler_job_runs`，不清理 `worker_heartbeats`；容器被 SIGKILL 时 `_heartbeat_loop` 无法 graceful shutdown，残留 status=running 僵尸记录导致管理员面板 Worker 健康状态不可信 | `_recovery_watchdog_loop` 同时清理 `worker_heartbeats` 僵尸 running 记录；阈值 600s；不删除记录、不涉及 migration；测试覆盖 8 个场景 | `CLOSED` | `backend/app/worker.py` 新增 `STALE_HEARTBEAT_THRESHOLD_SECONDS=600` 常量与 `mark_stale_worker_heartbeats` 函数，`_recovery_watchdog_loop` 同事务调用 `recover_stale_scheduler_job_runs` + `mark_stale_worker_heartbeats` 后 commit；`backend/tests/test_worker_heartbeat_stale_cleanup.py` 8 passed（fresh 心跳不处理、stale running 标记 stopped、非 running 不处理、批量处理、返回值匹配、阈值边界、watchdog 集成、幂等）；CHANGE-20260703-016 |
...
| GitHub Actions CI（release/docs-aligned-candidate-v3 最终 HEAD） | 已完成 | Run #42 最终 HEAD `aab2c72` 全部 blocking jobs success；非阻断的 Ruff/Mypy Full Repository Report 失败为预期历史债务展示；URL: `https://github.com/bao1872/market_dev/actions/runs/28658432580` |
| 生产部署验证 | 未验证 | 未执行 |
