...
- 禁止通过本地或远程 Shell 环境、SSH 命令参数或进程参数传输 `VERIFY_DB_URL`；禁止正式 runner 依赖宿主机 `psql`、Alembic 或 pytest；
...
- **自动 Fast CI / CI Gate 触发部署**；
- **未实现的多阶段 delivery 状态机**。
