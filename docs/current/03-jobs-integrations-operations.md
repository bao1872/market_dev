...
qstock 板块同步是独立每日任务（PRD §7.5），用于同步 A 股行业/概念板块目录和成分股关系到 `market_boards` / `market_board_memberships` 表，供 `/market/stocks` 的 `industry`/`concept` 筛选使用。

**当前状态：已接入 `worker-bars-scheduler` 调度**（17:00 CronTrigger，`max_instances=1`），但默认通过 `BOARD_SYNC_ENABLED` 关闭执行。服务层 `board_sync_service.sync_boards()` 和拉取适配器 `qstock_fetcher.QStockFetcher` 已实现并注册在 `run_bars_scheduler_worker()` 内的 `AsyncIOScheduler`。

### 2.6.0 BOARD_SYNC_ENABLED 开关与 provider 现状

- 新增配置 `BOARD_SYNC_ENABLED`（默认 `false`）：当 `false` 时 `scheduled_board_sync` 直接跳过执行，不发起任何 THS 请求，记录 `scheduler_job_runs.status='skipped'` + `reason_code='board_provider_unavailable'`；当 `true` 时按原流程执行同步；
- 生产仅在 THS provider 可用时才设置 `BOARD_SYNC_ENABLED=true`；
- **ALIGN-041（OPEN）**：当前物理机出口 IP 被 THS 反爬拦截（member 端点返回 403），akshare 当前无 THS member 端点，qstock/THS 板块目录拉取不可用，因此默认 `BOARD_SYNC_ENABLED=false`，`/market/boards` 返回 `available=false` + `reason_code='board_provider_unavailable'`；
- **Provider 链**：`sync_boards` 接收单一 `BoardFetcher` 实例，禁止 provider 混用或 fallback 链；THS 不可用时直接跳过，不切换到 akshare 等其他源；
- **HTTP 重试**：仅在 adapter 层（`_http_get_with_retry`）做单层重试，禁止在 service 层再包一层重试导致重试放大；
...
board_sync_service.sync_boards()
  → QStockFetcher.fetch_boards() 拉取行业 + 概念板块目录
  → QStockFetcher.fetch_memberships() 逐板块拉取成分股代码
  → 暂存到内存 StagingData
  → validate_staging_data() 完整性校验
  → 事务内原子切换（TRUNCATE + INSERT）
  → caller 显式 commit
```

- `qstock_fetcher.QStockFetcher` 延迟导入 qstock，不成为用户请求链运行时依赖；
- qstock 只存在于本适配器模块，用户请求链不直接调用 qstock；
- `validate_staging_data` 和 `prepare_atomic_swap` 是纯函数，可直接单元测试。

### 2.6.2 完整性门禁（V1.1）

校验项（任一失败即拒绝切换，保持上一成功版本）：

| 校验项 | 阈值 | 说明 |
|---|---|---|
| 空集合 | boards/memberships 非空 | 空目录或空成分拒绝 |
| 目录数下限 | `MIN_BOARD_COUNT = 100` | A 股至少 100+ 行业/概念板块 |
| 成分数下限 | `MIN_MEMBERSHIP_COUNT = 3000` | 至少 3000 条成分关系 |
| 异常降幅 | `MAX_DROP_PERCENT = 0.20` | 板块数/成分数较上次降幅 >20% 拒绝；首次同步（prev=0）不做降幅检查 |

### 2.6.3 事务边界与失败处理

- **失败保持上一成功版本**：校验失败或异常时不删除旧关系，旧数据保留可用；
- 原子切换在单个事务内执行（`delete MarketBoardMembership` → `delete MarketBoard` → `insert` 新数据），caller 显式 commit；
...
- `BOARD_SYNC_ENABLED=false` 时 job 仍被触发但立即跳过（`status=skipped`，`reason_code=board_provider_unavailable`），不发任何 HTTP 请求；
- 两个 job 独立调度，失败互不影响（board_sync 失败/跳过保留旧板块数据）；
