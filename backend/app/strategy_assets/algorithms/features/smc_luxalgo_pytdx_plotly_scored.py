...
class SMCIndicatorPineCloser:
    ...
    def display_structure(self, i: int, internal: bool = False):
        ...
        if crossed_up and (not piv.crossed) and extra_condition:
            ...
            if tag == CHOCH:
                ...
                if internal:
                    self.internal_bullish_bos_times.append(self.times[i])
                else:
                    self.swing_bullish_bos_times.append(self.times[i])
        ...
        if crossed_down and (not piv.crossed) and extra_condition:
            ...
            if tag == CHOCH:
                if internal:
                    self.internal_bearish_choch_times.append(self.times[i])
                else:
                    self.swing_bearish_choch_times.append(self.times[i])
    ...
    def _last_event_bars_since(self, ts_list: list[pd.Timestamp], current_i: int) -> int:
        ...
        last_ts = ts_list[-1]
        try:
            last_idx = self.times.index(last_ts)
        except ValueError:
            return 9999
        return max(0, current_i - last_idx)

    def _last_pivot_type(self, current: Pivot, is_high: bool) -> str:
        if pd.isna(current.currentLevel) or pd.isna(current.lastLevel):
            return "NA"
        if is_high:
    ...
    def _ob_metrics(self, ob: OrderBlock | None, close: float, atr_val: float) -> dict[str, float]:
        if ob is None or pd.isna(atr_val) or atr_val <= 0:
            return {
                ...
                "inside": 0,
                "width_atr": np.nan,
                "bars_since_created": 9999,
                "below_bottom": 0,
                "reclaim_mid": 0,
            }
        top = float(ob.barHigh)
        bottom = float(ob.barLow)
        mid = (top + bottom) / 2.0
        prev_close = np.nan
        if self._factor_build_i > 0:
            prev_close = float(self.closes[self._factor_build_i - 1])
        return {
            "top": top,
            "bottom": bottom,
            "mid": mid,
            "dist_mid_atr": abs(close - mid) / atr_val,
            "inside": int(bottom <= close <= top),
            "width_atr": (top - bottom) / atr_val,
            "bars_since_created": max(0, self._factor_build_i - ob.barIndex),
            "below_bottom": int(close < bottom),
    ...
    def build_factor_row(self, i: int) -> dict[str, object]:
        ...
        atr_val = float(row["atr200"]) if pd.notna(row["atr200"]) else np.nan

        last_swing_low_type = self._last_pivot_type(self.swingLow, is_high=False)
        last_swing_high_type = self._last_pivot_type(self.swingHigh, is_high=True)

        bars_since_internal_bullish_choch = self._last_event_bars_since(self.internal_bullish_choch_times, i)
        bars_since_swing_bullish_bos = self._last_event_bars_since(self.swing_bullish_bos_times, i)
        recent_internal_bearish_choch = int(self._last_event_bars_since(self.internal_bearish_choch_times, i) <= 5)

        internal_bull_ob = self._nearest_ob(internal=True, bias=BULLISH)
        internal_bear_ob = self._nearest_ob(internal=True, bias=BEARISH)
        internal_bull = self._ob_metrics(internal_bull_ob, close, atr_val)

        if internal_bear_ob is not None and pd.notna(atr_val) and atr_val > 0:
            nearest_internal_bear_ob_mid = (internal_bear_ob.barHigh + internal_bear_ob.barLow) / 2.0
            dist_to_nearest_internal_bear_ob_atr = (nearest_internal_bear_ob_mid - close) / atr_val
        else:
            nearest_internal_bear_ob_mid = np.nan
            dist_to_nearest_internal_bear_ob_atr = np.nan

        trailing_top = float(self.trailing.top) if pd.notna(self.trailing.top) else np.nan
        trailing_bottom = float(self.trailing.bottom) if pd.notna(self.trailing.bottom) else np.nan
        if pd.notna(trailing_top) and pd.notna(trailing_bottom) and trailing_top > trailing_bottom:
            close_pos_in_trailing_range = (close - trailing_bottom) / (trailing_top - trailing_bottom)
            close_pos_in_trailing_range = float(np.clip(close_pos_in_trailing_range, 0.0, 1.0))
        else:
            close_pos_in_trailing_range = np.nan

        score_trend = 0.0
        if self.swingTrend.bias == BULLISH:
            score_trend += 15
        if self.internalTrend.bias == BULLISH:
            score_trend += 5
        if last_swing_low_type == "HL":
            score_trend += 8
        if last_swing_high_type == "HH":
            score_trend += 7

        score_ob = 0.0
        inside_internal_bull_ob = internal_bull["inside"]
        dist_to_internal_bull_ob_mid_atr = internal_bull["dist_mid_atr"]
        internal_bull_ob_width_atr = internal_bull["width_atr"]
        bars_since_internal_bull_ob_created = internal_bull["bars_since_created"]
        close_below_internal_bull_ob_bottom = internal_bull["below_bottom"]
        close_reclaim_internal_bull_ob_mid = internal_bull["reclaim_mid"]

        if inside_internal_bull_ob == 1:
            score_ob += 12
        elif pd.notna(dist_to_internal_bull_ob_mid_atr):
            if dist_to_internal_bull_ob_mid_atr <= 0.5:
                score_ob += 10
            elif dist_to_internal_bull_ob_mid_atr <= 1.0:
                score_ob += 6
            elif dist_to_internal_bull_ob_mid_atr <= 1.5:
                score_ob += 2

        if pd.notna(internal_bull_ob_width_atr):
            if 0.3 <= internal_bull_ob_width_atr <= 1.5:
                score_ob += 5
            elif internal_bull_ob_width_atr <= 2.5:
                score_ob += 2
            else:
                score_ob -= 2

        if bars_since_internal_bull_ob_created <= 10:
            score_ob += 4
        elif bars_since_internal_bull_ob_created <= 20:
            score_ob += 2

        if close_below_internal_bull_ob_bottom == 0 and pd.notna(internal_bull["bottom"]):
            score_ob += 4
        score_ob = min(score_ob, 35.0)

        score_trigger = 0.0
        if bars_since_internal_bullish_choch <= 3:
            score_trigger += 8
        elif bars_since_internal_bullish_choch <= 7:
            score_trigger += 5
        elif bars_since_internal_bullish_choch <= 12:
            score_trigger += 2

        if bars_since_swing_bullish_bos <= 5:
            score_trigger += 6
        elif bars_since_swing_bullish_bos <= 12:
            score_trigger += 3

        if close_reclaim_internal_bull_ob_mid == 1:
            score_trigger += 4

        if (row["high"] > row["low"]) and (abs(row["close"] - row["open"]) / (row["high"] - row["low"]) >= 0.4) and (row["close"] > row["open"]):
            score_trigger += 2
        score_trigger = min(score_trigger, 20.0)

        score_risk = 0.0
        if close_below_internal_bull_ob_bottom == 1:
            score_risk -= 10
        if pd.notna(dist_to_nearest_internal_bear_ob_atr):
            if 0 <= dist_to_nearest_internal_bear_ob_atr < 0.8:
                score_risk -= 6
            elif 0.8 <= dist_to_nearest_internal_bear_ob_atr < 1.5:
                score_risk -= 3
        if pd.notna(close_pos_in_trailing_range):
            if close_pos_in_trailing_range > 0.85:
                score_risk -= 4
            elif close_pos_in_trailing_range > 0.75:
                score_risk -= 2
        if recent_internal_bearish_choch == 1:
            score_risk -= 4

        score_total = score_trend + score_ob + score_trigger + score_risk
        is_candidate = int(
            (self.swingTrend.bias == BULLISH)
            and ((bars_since_internal_bullish_choch <= 20) or (bars_since_swing_bullish_bos <= 20))
            and (close_below_internal_bull_ob_bottom == 0)
            and (score_total >= 65)
        )
        ...
        return {
            ...
            "vol": float(row["vol"]),
            "atr200": atr_val,
            "swing_trend_bias": int(self.swingTrend.bias),
            "internal_trend_bias": int(self.internalTrend.bias),
            "last_swing_low_type": last_swing_low_type,
            "last_swing_high_type": last_swing_high_type,
            "bars_since_internal_bullish_choch": int(bars_since_internal_bullish_choch),
            "bars_since_swing_bullish_bos": int(bars_since_swing_bullish_bos),
            "recent_internal_bearish_choch": recent_internal_bearish_choch,
            "nearest_internal_bull_ob_top": internal_bull["top"],
            "nearest_internal_bull_ob_bottom": internal_bull["bottom"],
            "nearest_internal_bull_ob_mid": internal_bull["mid"],
            "dist_to_internal_bull_ob_mid_atr": dist_to_internal_bull_ob_mid_atr,
            "inside_internal_bull_ob": inside_internal_bull_ob,
            "internal_bull_ob_width_atr": internal_bull_ob_width_atr,
            "bars_since_internal_bull_ob_created": int(bars_since_internal_bull_ob_created),
            "close_below_internal_bull_ob_bottom": close_below_internal_bull_ob_bottom,
            "close_reclaim_internal_bull_ob_mid": close_reclaim_internal_bull_ob_mid,
            "nearest_internal_bear_ob_mid": nearest_internal_bear_ob_mid,
            "dist_to_nearest_internal_bear_ob_atr": dist_to_nearest_internal_bear_ob_atr,
            "trailing_top": trailing_top,
            "trailing_bottom": trailing_bottom,
            "close_pos_in_trailing_range": close_pos_in_trailing_range,
    ...
    def run(self):
        for i in range(len(self.df)):
            ...
            self.factor_rows.append(self.build_factor_row(i))
    ...
    def figure(self, title: str) -> go.Figure:
        ...
        factor_df = pd.DataFrame(self.factor_rows) if self.factor_rows else pd.DataFrame(index=range(len(self.df)))
        x_vals = self.base_x_list
        ...
        fig.update_yaxes(showgrid=True, zeroline=True, row=2, col=1)
        return fig
