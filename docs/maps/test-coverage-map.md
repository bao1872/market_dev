...
| **[两阶段 compute/write 一致性 - CHANGE-20260711-001]** 旧路径 `compute_for_trade_date` 与新路径 `compute_records_for_trade_date` 逐字段深比较等价（正常/日线不足/缺 15m/复权变化 4 场景 × 10 股）；compute 阶段只读（monkeypatch `upsert_snapshot`/`bulk_upsert_records` 调即失败，外部 reader session 查 0 行）；write 原子性（成功 commit 后一次性可见；第 2 批人为抛异常 rollback 后 0 条新数据，旧 published 仍完整）；loader 批量查询边界（`loader_batch_size=20` 时 20 股=3 次批量查询、40 股=6 次，常数次不逐股）；50 股 compute-only smoke（RSS<1800MB 且无失控增长） | `test_feature_snapshot_consistency.py`（9 个用例：old_vs_new_normal/insufficient_daily/missing_15m/adj_change + compute_phase_external_invisible + write_phase_commit_visible + write_phase_failure_rollback + loader_batch_query_constant_calls + compute_only_50_smoke） |
...
| 盘后流水线聚合 API（12 场景）：盘前 not_started/收盘后 blocked/latest 不回退历史/运行中 5 阶段/成功/watchlist_ready 判定/sample 不计入/full 优先展示/失败带 error/feature_snapshot_stalled/POST 幂等/events 限 100 条/非 admin 403 | `test_admin_after_close_pipeline.py`（12 个用例）+ `test_after_close_pipeline_service.py`（8 个用例：5 阶段映射/负耗时归零/虚拟步骤归并/停滞判定/进度事件每间隔落库） |
