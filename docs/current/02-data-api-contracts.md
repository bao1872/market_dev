...
- active swing 位置（`price_position_in_active_swing_0_1`）clip 到 [0,1]，反映当前正在发展的结构区间，**是前端摘要卡唯一位置字段**；
...
| 字段 | 公式 |
|---|---|
| m15_position_relative_to_daily | `m15_price_position_in_active_swing_0_1 - daily_price_position_in_active_swing_0_1`（V1.9 起改用 active swing，不再用 confirmed raw，避免强趋势段 -1.755 等极端值） |
...
- `daily_price_position_in_active_swing_0_1` = `primary.swing_position.price_position_in_active_swing_0_1`（clip [0,1]，**摘要卡位置字段事实源**）
...
- `m15_price_position_in_active_swing_0_1` = `secondary.swing_position.price_position_in_active_swing_0_1`（clip [0,1]，**摘要卡位置字段事实源**）
...
V1.9 derived_relation 口径变更：
- `m15_position_relative_to_daily` 改用 active swing 位置（`m15_price_position_in_active_swing_0_1 - daily_price_position_in_active_swing_0_1`），不再使用 confirmed raw 相减；
- 原因：confirmed raw 在强趋势段会出现 >1 或 <0 的极值，相减后产生 -1.755 等无意义值；active swing clip [0,1] 后位置差值范围 [-1, 1]，语义更稳定；
