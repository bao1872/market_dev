...
- 配置 `BOARD_SYNC_ENABLED`（CHANGE-20260716-007：环境变量注入，默认 `false`）：`config.py` 的 `_resolve_board_sync_enabled()` 优先级为「环境变量 > CONFIG_FILE > 默认 False」；`docker-compose.prod.yml` 的 worker-after-close 服务显式注入 `BOARD_SYNC_ENABLED: ${BOARD_SYNC_ENABLED:-false}`；环境变量接受 `1`/`true`/`yes`/`on`（大小写不敏感）为真，其余为假；当 `false` 时 `syncing_boards` 步骤直接跳过执行，不发起任何 pywencai 请求，记录 `scheduler_job_runs.status='skipped'` + `reason_code='board_sync_disabled'`；当 `true` 时按流程执行同步；
