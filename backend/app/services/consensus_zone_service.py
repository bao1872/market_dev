...
def compute_data_version(
    ...
    基于排序、标准化后的 1d 和 15m 时间戳 + OHLCV 摘要生成 SHA256 前 8 字符。
    不依赖 DataFrame 字符串表示（不稳定），而是提取标量值后拼接。

    稳定性保证：
    - 时间戳排序后取首尾 ISO + 数量
    - OHLCV 摘要：首尾 close、总 volume、max high、min low
    - 相同数据（即使行序不同）产生相同 hash

    Args:
        daily_bars: 日线 DataFrame
        bars_15min: 15m DataFrame（可为 None）
    ...
    if not daily_bars.empty:
        dt_col = "datetime" if "datetime" in daily_bars.columns else (
            "trade_date" if "trade_date" in daily_bars.columns else None
        )
        if dt_col:
            sorted_bars = daily_bars.sort_values(dt_col)
            times = pd.to_datetime(sorted_bars[dt_col])
            first_ts = times.iloc[0].isoformat()
            last_ts = times.iloc[-1].isoformat()
            count = len(sorted_bars)
            # OHLCV 摘要（标量，不依赖行序）
            first_close = float(sorted_bars["close"].iloc[0])
            last_close = float(sorted_bars["close"].iloc[-1])
            total_vol = float(sorted_bars["volume"].sum())
            max_high = float(sorted_bars["high"].max())
            min_low = float(sorted_bars["low"].min())
            parts.append(
                f"d:{count}:{first_ts}:{last_ts}:"
                f"{first_close:.4f}:{last_close:.4f}:"
                f"{total_vol:.2f}:{max_high:.4f}:{min_low:.4f}"
            )
        else:
            parts.append(f"d:{len(daily_bars)}:notime")

    if bars_15min is not None and not bars_15min.empty:
        dt_col = "datetime" if "datetime" in bars_15min.columns else (
            "trade_date" if "trade_date" in bars_15min.columns else None
        )
        if dt_col:
            sorted_bars = bars_15min.sort_values(dt_col)
            times = pd.to_datetime(sorted_bars[dt_col])
            first_ts = times.iloc[0].isoformat()
            last_ts = times.iloc[-1].isoformat()
            count = len(sorted_bars)
            ...
            min_low = float(sorted_bars["low"].min())
            parts.append(
                f"m:{count}:{first_ts}:{last_ts}:"
                f"{first_close:.4f}:{last_close:.4f}:"
                f"{total_vol:.2f}:{max_high:.4f}:{min_low:.4f}"
            )
        else:
            parts.append(f"m:{len(bars_15min)}:notime")
...
async def compute_and_cache_consensus_zone(
    ...
    服务入口内部执行因果性过滤（filter_bars_by_as_of），
    调用方传入的 bars 可能包含未来数据，由本函数统一过滤。
