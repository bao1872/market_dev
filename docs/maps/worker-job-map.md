...
|---|---|---|---|
| worker-bars-scheduler | bars_scheduler | bars*, strategy_runs, scheduler_job_runs, company_actions | **Phase 8A**：16:00 不再直接刷新行情和触发 DSA，改为调用 `create_after_close_run` 创建/复用 after-close run（幂等）；行情刷新由 orchestrator `refreshing_daily` 步骤执行（`trigger_dsa=False`）；复用 `BarsCoverageService` 统一口径，业务日期使用 `shanghai_business_date()`；16:00 CronTrigger + `max_instances=1` AsyncIOScheduler；**板块同步已迁移至 `worker-after-close` 的 `syncing_boards` 步骤**（CHANGE-20260716-007）；**盘后顺序门禁（CHANGE-20260717-002）**：原始日线刷新 → 公司行为/factor 重建成功 → 覆盖率门禁/DSA → snapshot 发布；因子未完成时不得创建 DSA 或发布 snapshot |
| worker-strategy-scheduler | strategy_scheduler | strategy_runs | **Phase 8A**：18:30 对 `dsa_selector` 策略调用 `create_after_close_run` 创建/复用 after-close run（幂等，16:00 已创建则跳过）；非 DSA selector 策略仍走 `create_batch_run(run_type="scheduled")` 由 generic worker 领取；不再进入旧"DSA 完成后触发 after-close"路径 |
...
| worker-capture | capture service | capture_jobs, notification_messages | 截图失败但状态不可见；**调用方**：`monitor_batch_service._send_chart_images_via_outbox()`、`notification_service.test_channel_latest_event()`、`stock_detail_feishu_service`；**capture token 必须含** `scope=stock_detail_capture` / `user_id` / `instrument_id` / `event_id`，否则 `/api/v1/capture/stocks/{instrument_id}/snapshot` 返回 401/403；`page.goto` 使用 `wait_until="load"`（修复 `networkidle` 在前端长连接下 30s 超时返回 502 的问题），并通过 `data-render-ready="true"` 等待 bars+indicators 就绪；截图成功返回 `image_url`，失败由调用方写 `capture_jobs.status=failed`；**三类 IndicatorView 独立飞书图片（CHANGE-20260720-001 §三）**：`CaptureJob` 新增 `indicator_view: Literal["node_cluster", "bollinger", "smc"]` 字段；缓存键含 `indicator_view`（不同 IndicatorView 缓存隔离）；输出文件名含 `indicator_view`（如 `capture_<symbol>_<indicator_view>_<run_id>.png`）；幂等键含 `indicator_view`（同股票同 IndicatorView 同事件去重）；三套 `FEISHU_CAPTURE_PRESETS` dict（`node_cluster`/`bollinger`/`smc`）layers 互斥（除共享 candlestick），`indicator_view` 透传到 URL 参数；状态查询 `get_share_status` 返回 `indicator_view` |
