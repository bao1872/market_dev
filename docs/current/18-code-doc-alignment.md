...
| ALIGN-014 | CI | `ruff`/`type-check` job 曾设置 `continue-on-error: true`，失败不阻断 workflow；`Ruff Changed Files` 相对 `main` 检查整个大型 feature diff 会失败；旧 `type-check` 任务因 252 个历史 mypy 错误直接阻断合并 | release 候选最终 HEAD 的 blocking jobs 全绿，新增文件 Ruff/mypy 零错误，历史债务不新增/不增加/总数不超基线 | `CLOSED` | GitHub Actions run [#34](https://github.com/bao1872/market_dev/actions/runs/28653543329) 最终 HEAD `69e7b70` 全部 blocking jobs 成功（Architecture Rules、Docs Consistency、Test Allowlist、Ruff New Files、Ruff Baseline Regression、Mypy New Files、Mypy Baseline Regression、Alembic Cycle、PostgreSQL Integration Tests、Frontend Type Check、Frontend Lint、Frontend Build）；三层 Ruff/Mypy 配置生效，历史债务未新增/未增加/总数未超基线；见 CHANGE-20260702-010、CHANGE-20260702-011 |
...
| ALIGN-018 | Capture 链路 | Phase C 已实现专用 `/capture/stock/:symbol` 前端路由 + `/api/v1/capture/stocks/{instrument_id}/snapshot` 后端 API + Capture Token 隔离；但 `CaptureStockPage.tsx` 此前仍调用普通端点，`stock_capture_service.py` URL 未携带 `instrument_id`，状态机字段不完整 | 专用 `/capture/stock/:symbol` 路由 + Capture Token 校验 + Snapshot API 唯一数据请求 + URL 携带 `instrument_id` + 完整状态机字段 | `CLOSED` | CHANGE-20260702-011 已重构 `CaptureStockPage.tsx` 仅调用 Snapshot API、`stock_capture_service.py` URL 携带 `instrument_id`、补齐 `capture_status`/`image_upload_status`/`overall_status` 状态机；`tests/test_capture_snapshot.py`、`tests/test_capture_token_isolation.py`、`tests/test_state_machine.py` 通过；frontend contract 52 passed；GitHub Actions run [#34](https://github.com/bao1872/market_dev/actions/runs/28653543329) 全部 blocking jobs 成功；真实飞书生产 E2E 待 Phase F 部署后执行 |
...
| 验证层级 | 状态 | 说明 |
|---|---|---|
| 本地代码/测试/文档检查 | 已完成 | pytest 1106 passed（CI）、frontend contract 52 passed、`tools/update_docs.py --check`、`check_architecture.py`、`check_docs_consistency.py`、`check_test_allowlist.py` 通过；Ruff 新增文件零错误、Mypy 新增文件零错误、基线无回归 |
| GitHub Actions CI | 已完成 | run [#34](https://github.com/bao1872/market_dev/actions/runs/28653543329) 最终 HEAD `69e7b70` 全部 blocking jobs success |
| 生产部署验证 | 未验证 | 未执行 |
