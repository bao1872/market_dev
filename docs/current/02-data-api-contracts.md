...
| 行情 | `/instruments`, `/calendar`, `/market`, `/bars` | 数据新鲜度、partial/degraded 标识；`/instruments/{id}/bars` page_size 按 timeframe 限制：`15m` 最大 4000，`1h` 最大 1200，其他最大 1000；`/instruments/{id}/indicators` 的 `bars` 参数最大 4000；`indicators` 响应含 `sqzmom_lb` 全局技术指标数据，后端逐行复刻 Pine 代码，前端只渲染不计算；**SMC 指标按需启用**（`include_smc` 查询参数，默认 `false`，`true` 时返回 `data.smc` 且 cache key 追加 `:smc` 后缀，详见第 9.5 节）；FVG 完全排除；**`GET /market/stocks` 行业/概念筛选**（PRD §7.5 pywencai 同步后）：`industry`/`concept` 参数按板块名称精确匹配筛选（不支持模糊匹配），通过 `filter_instruments_by_board()` 查询 `market_boards` 表实现；未同步板块数据时返回空列表（不报 422）；`industry` + `concept` 同时传时取交集（AND 语义）；行业值 `-` 在 API 层不变（前端可渲染为 `/`）；**`GET /market/boards?type=industry\|concept`** 返回 `MarketBoardsResponse`：`items`（板块目录列表）+ `available: bool`（items 非空时 true）+ `reason_code: str \| null`（无数据时为 `"board_provider_unavailable"`）+ `updated_at` + `source: str \| null`（数据源标识，如 `"wencai"`）+ `stale: bool`（存在旧数据但最新同步失败时为 `true`，前端展示"沿用上次板块数据"）+ `last_attempt_status: str \| null`（最近一次同步状态：`succeeded`/`failed`/`validation_failed`/`skipped`）；前端依据 `available` 决定筛选输入是否禁用，依据 `stale` 决定是否展示"沿用上次板块数据"提示 |
