...
**[RC4] checkpoint 单调性**：共享覆盖率步骤成功时**只**刷新 heartbeat/lease（`last_completed_step=None`），绝不写 `refreshing_daily`/`checking_coverage`。否则 resume 时会把已有的更晚 checkpoint（如 `computing_features`）倒退成 `refreshing_daily`，crash 后错误重算 features。真正的 `refreshing_daily` checkpoint 只由 `refreshing_daily` 刷新步骤自身在成功后写一次（位于 `if not skip_refresh:` 分支内）。实现上覆盖率步骤抽为模块级 `_run_shared_checking_coverage_step(...)`（normal/resume/restart 共享，行为可单测）。

**[RC5] board 级合同一致性**：`validate_envelope()` 在 envelope.contracts 校验通过后，进一步强制每个 board 自带的 `source` / `taxonomy` / `taxonomy_version` / `taxonomy_compatibility_key` / `identity_contract_version` 与冻结合同一致（仅"非空"不足：`identity_contract_version="garbage-v999"` 会被写进 PIT 不可变身份）。`build_envelope()` 同样校验（本地早失败）。

**[RC6] formal 注册**：本地手动板块导入的 PG 证据已注册为 required targeted-pg 合同 `board_local_snapshot_import`（selector `tests/test_board_snapshot_import_pg.py`）。
