...
- 配置 `BOARD_SYNC_ENABLED`（默认 `true`）：当 `false` 时 `syncing_boards` 步骤直接跳过执行，不发起任何 pywencai 请求，记录 `scheduler_job_runs.status='skipped'` + `reason_code='board_sync_disabled'`；当 `true` 时按流程执行同步；
