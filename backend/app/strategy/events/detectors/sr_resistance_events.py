...
从 ref/交易/event_lib/detectors/sr_resistance_events.py 迁移。
升级：添加 state_ttl_seconds 和 allowed_roles 声明。

Registered Events:
    - evt_high_break_recent_resistance: 盘中突破压力
    - evt_cross_recent_resistance: 收盘上穿压力（TRIGGER/CONFIRM）
    - evt_wick_break_resistance_fail: 影线突破失败（VETO）
    - evt_gap_up_break_resistance: 跳空突破压力
    - evt_break_resistance_bull_bar: 突破压力+阳线
    - evt_break_resistance_close_strong: 突破压力+收盘强势
    - evt_break_resistance_from_low_zone: 低位突破压力（TRIGGER/CONFIRM）
    - evt_break_resistance_from_mid_zone: 中位突破压力
    - evt_break_strong_resistance_cluster: 突破强压力簇（TRIGGER/CONFIRM）
    - evt_wick_break_resistance_cluster_fail: 强压力簇影线突破失败（VETO）
    - evt_close_above_resistance_cluster_upper: 收盘站上压力簇上界（TRIGGER/CONFIRM）
"""
