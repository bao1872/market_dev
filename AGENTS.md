...
docs/current/08-indicator-calculation-contracts.md
...
22. 未经许可直接修改生产环境任何账户密码（包括管理员与普通用户）。
23. 生产代码、测试、工具、构建脚本在运行时 import/open/read/glob `ref/` 目录，或把 `ref/` 文件称为真源/合同/fixture 生成器/运行依赖（`ref/` 仅允许人工阅读，详见 clause 59/60）。
...
34. 盘迹品牌视觉 V1.0（CHANGE-20260713-006）：视觉参考源（人工阅读）为 `ref/盘迹品牌视觉资产包_v1.0/`；`frontend/src/styles/variables.scss` 为唯一视觉 token 真源，禁止在组件中硬编码颜色（必须使用 `v.$color-*` 或 `var(--*)`）；品牌主色为莹感绿 `#00F6C2`（`$color-brand`），只承担品牌焦点和关键交互（主按钮、选中 tab、focus 轮廓、Logo 末端节点），不得用于表达涨跌；A股继续红涨绿跌（`$color-up` `#FF4D4F` / `$color-down` `#22C55E`）；大面积背景使用深石墨黑 `#0A0F14`（`$color-bg`），避免荧光绿铺满页面；`BrandLogo` 为四节点折线路径 + 末端高亮共识节点（莹感绿圆环），不变形、不旋转、不增减节点、不替换颜色；Logo 资产位于 `frontend/src/assets/brand/`（`logo_symbol_128.png` / `logo_symbol_256.png` / `logo_horizontal_dark.png`），ref 路径不作为运行时依赖；中文字体 MiSans/HarmonyOS Sans SC/PingFang SC，fallback Noto Sans CJK SC，数字用等宽字体；卡片圆角 10-14px、1px 边框；禁止重阴影和大面积玻璃拟态；视觉改造不得改变 DSA、Node Cluster、盘中监控、Capture 计算口径。
...
53. TV parity baseline + SMC source diagnostics（CHANGE-20260716-001；CHANGE-20260718-004 ref 隔离修正）：（1）**Pine 隐藏 export 字段（CHANGE-20260716-001）**：**参考源 `ref/smc_user_source.pine`（SHA256 0bd3d2ad，843 行，人工阅读，非运行依赖）不可变**，导出功能在派生文件 `ref/smc_user_export.pine`（CHANGE-20260718-004：已 `git rm --cached`，不再纳入 git 跟踪，仅人工阅读）末尾新增 18 个 `plot(..., display=display.none)` 隐藏 export 字段——`_exp_open`/`_exp_high`/`_exp_low`/`_exp_close`（OHLC）、`_exp_int_bull_bos`/`_exp_int_bear_bos`/`_exp_int_bull_choch`/`_exp_int_bear_choch`（internal BOS/CHoCH）、`_exp_swing_bull_bos`/`_exp_swing_bear_bos`/`_exp_swing_bull_choch`/`_exp_swing_bear_choch`（swing BOS/CHoCH）、`_exp_int_bull_ob`/`_exp_int_bear_ob`（internal OB）、`_exp_eqh`/`_exp_eql`（EQH/EQL）、`_exp_swing_bias`/`_exp_int_bias`（bias）；`display=display.none` 不影响图表和算法，但数据可通过 TV "Export indicator data" 导出为 CSV。（2）**CSV fixture 路径**：`backend/tests/fixtures/smc_pine/smc_tv_<symbol>_<tf>.csv`；项目 Python 测试直接读取该 CSV，**禁止从 DB 重新取另一套 Bar**；产品继续默认前复权，TV parity fixture 使用与 TV 完全相同的复权方式、数据源和 completed-bar 边界。（3）**parity 测试** `backend/tests/test_smc_tv_parity.py`（`PINE_PARITY_PENDING`）：`test_tv_csv_bar_parity`（断言 time/OHLC/bar 数量逐项相等，浮点容差 1e-8，不相等写 `INPUT_BAR_MISMATCH`，**不得调整算法迎合截图**）、`test_tv_csv_event_parity`（比较事件有序序列，bar_index ±1 容差）、`test_tv_csv_swing_bias_parity`（比较最后一根 bar 的 swing_bias）；无 CSV fixture 时 skip，不得宣称"完全对齐"。（4）**SMC source 诊断字段（CHANGE-20260716-001）**：API 新增诊断字段（仅 `include_smc=True` 时返回）——`smc_source_bar_hash`（基于 SMC 实际完整输入 `smc_bars` 计算 hash；1d 用 `full_daily_bars`，其他用 `macd_bars`；**不得复用截断后的 `macd_bars` hash**）、`smc_source_first_time`、`smc_source_last_time`、`smc_source_bars`、`smc_adj`；`include_smc=False` 时所有 `smc_source_*` 字段为 None/0。（5）**000725 项目回归基线（CHANGE-20260718-004）**：京东方A 000725 的 SMC 输出（17 events / 21 OB / 2 EQL / swing_bias=1）是**项目回归基线**，**不称 TV golden 或完全对齐**；`PINE_PARITY_PENDING` 保留至 TradingView CSV 提供；回归基线用于验证 SMC 算法无回归（输入一致 + 输出序列稳定），不用于宣称与 TV 逐 bar 对齐。

54. 详情页导航 originScope 单一真源（CHANGE-20260716-006）：（1）**单一 URL builder**：`frontend/src/features/stock-research/stockDetailNavigation.ts` 提供 `buildStockDetailUrl`/`resolveStockDetailOrigin`/`sourceForOriginScope`/`strategyForOriginScope`，为详情页导航唯一 URL 构造入口；禁止第二套 URL 手工拼接；`detailNavigation.ts` 旧 `buildStockDetailUrl` 已删除，不得恢复。（2）**originScope 优先级**：显式 `originScope` 参数（`market`/`watchlist`）为来源第一真源，不被 `returnTo.scope` 覆盖；旧 URL 无 `originScope` 时兼容解析 `returnTo.scope`；无任何来源的直接 URL 默认 `watchlist`。（3）**冲突检测**：`originScope` 存在且 `returnTo.scope` 不同 → `contextMismatch=true`，禁止静默回退自选；前端必须显式渲染冲突状态，不得伪装成单一来源。（4）**三入口统一**：`MarketWorkspacePage` 点击股票名称、`useStockDetailActions` 上一只/下一只、`StockDetailPage` 左栏来源列表点击，全部调用同一 `buildStockDetailUrl`；`MarketWorkspacePage` 使用当前 `searchParams` 副本并强制写入 `scope` 和 `selected` 生成 returnTo，禁止直接复制可能滞后的 `location.search`。（5）**returnTo 职责**：`returnTo` 只负责返回导航和筛选状态恢复（`scope/selected/sort/keyword/filters/page/page_size/industry/concept`），不得作为来源判定真源；来源判定只看 `originScope`。

55. AFC 产品观察扩展不修改冻结 Core 14 / worker 持久化链（CHANGE-20260716-006）：（1）**独立第三合同**：`backend/app/contracts/atomic_fact_product_observations_v1.json` 为独立产品观察合同，sourceFields 仅 `raw`/`confirmed_high`/`confirmed_low`；`inside` 由 `compute_product_observations` 纯函数从已存在 `price_position_in_confirmed_swing_raw` 派生（`inside = raw if 0<=raw<=1 else None`），**不读取 worker 持久化的 `_0_1` 字段**，区间外数值不静默 clip（<0 显示"低于确认区间"，>1 显示"高于确认区间"）。（2）**不修改 worker**：`structural_factor_service.py` 不得新增 `price_position_in_confirmed_swing_0_1` 持久化字段，worker 持久化链保持不变；产品观察由 API 层从已有 `structural_payload.primary.1d` 字段派生，worker 旧镜像不需要升级即可支持。（3）**不计入 Core 14**：产品观察不进入 `core`/`auxiliary`/`availability`，不影响 14/14 统计；观察项 `publicKey=confirmed_swing_position`、`visualKind=confirmed_position`、`scope=product`；confirmed_swing_position 变化在 recentChanges 中标 `scope=product`，不得伪装 V4.13 Core Fact。（4）**Pydantic 模型**：新增 `ProductObservationItem`/`ProductObservations` Pydantic 模型；Context 响应新增 `productObservations.structure` 字段；前端 `ConfirmedPositionRow` 组件插入结构组"价格与已确认区间关系"之后，0.33/0.67 边界与 S3 一致；保留现有 S3"价格在当前主要波段的位置"（Active Swing 公式 clip [0,1]，Confirmed Swing 公式 raw 不 clip）。

56. 市场数据 SSOT 与复权唯一出口（CHANGE-20260717-002）：（1）**MDAS 唯一读取出口**：`MarketDataAggregationService.get_bars` 是后端唯一行情读取出口；业务/API/indicator/SMC/strategy_batch/feature_snapshot/structural_factor/temporal_feature/monitor/capture/chart_bars 全部经 MDAS，禁止业务层直接调用 `bar_repository` 的私有 `_query_*`/`_get_adj_factor_df`/`apply_adj_factor*` 或旧 `bar_repository.get_bars`；repository 仅负责 DB 读写原始 OHLCV 与公司行为因子，不做复权。（2）**raw 不可改 + 复权只做一次**：原始 bar 始终保持不复权落库；qfq 只在 MDAS 出口统一应用一次（`_apply_adj_factor_core`），禁止业务层二次复权或自行周/月聚合；MDAS 自身 resample 仅允许 1m→15min/60min 日内聚合，周/月聚合走 `kline_aggregator.convert_kline_frequency`；算法实现层（`strategy_assets/algorithms/`）在已获取的 bars 上自行 resample 计算算法内部特征（如 SMC PDH/PDL）不属于行情出口聚合，不受此约束。（3）**不信任 bar 自带 adj_factor 列**：`bars_15min`/`bars_60min`/`bars_minute` 行内 `adj_factor` 字段不可作为复权真源（pytdx hybrid bar 自带 `adj_factor=1.0` 错误）；列可保留兼容，但 MDAS 读取时必须由 `AdjustmentFactorService.get_factor_series` 返回的权威日线因子序列覆盖（`pd.merge_asof` 向量化映射到 `_adj` 列）；15m/60m/1m 日内 bar 按交易日映射到同一权威日线因子。（4）**adjustment_as_of point-in-time 截断**：`get_factor_series(session, instrument_id, as_of=date)` 返回只含 `trade_date <= as_of` 的截断因子序列；当前页面以请求业务日为锚，盘后/历史回算以 `trade_date` 为锚；公式 `qfq_price = raw_price × factor(bar_date) / factor(as_of)`；as_of 之后的除权事件不得泄漏到历史回算中。（5）**factor rebuild 完整重建 + 失败不伪装**：公司行为集合或 fingerprint 变化时，`AdjustmentFactorService.rebuild_factor_series` 必须从最早受影响日期完整重建该股票日线 factor 序列并原子 upsert（禁止只更新最近 5 根）；rebuild 成功后精确失效该股票 MDAS/indicator 缓存；rebuild 失败不得用 `1.0` 伪装成功，必须返回 degraded 状态和原因，受影响结果不发布。（6）**盘后顺序门禁**：BarsScheduler 顺序为「原始日线刷新 → 公司行为/factor 重建成功 → 覆盖率门禁/DSA → snapshot 发布」；因子未完成时不得创建 DSA 或发布 snapshot；`feature_snapshot_service`/after-close 调用 MDAS 显式 `include_realtime=False, end_date=trade_date, adjustment_as_of=trade_date`，保存 `source_bar_hash`/`adj_factor_hash`/`contract_version`/`completed_through`/`adjustment_as_of` 到 run metadata。（7）**MDAS v2 契约**：请求参数含 `timeframe`/`adj`/`include_realtime`/`completed_only`/`start_date`/`end_date`/`limit`/`warmup_bars`/`adjustment_as_of`；返回 bars 及 `market_data_contract_version`/`source_bar_hash`/`adj_factor_hash`/`adjustment_as_of`/`completed_through`/source/degraded 诊断字段；缓存键包含全部参数和版本；同一股票/周期/截止日下 `/bars`、indicator/SMC、strategy_batch、feature_snapshot 的 OHLC 时间序列、`source_bar_hash`、`adj_factor_hash` 必须一致。（8）**架构守护 AST 测试**：`backend/tests/test_market_data_ssot_architecture.py`（5 项测试）扫描 `app/` 全部生产文件，禁止除 MDAS 和 repository 外的生产模块导入 repository 私有 `_query_*`/`_get_adj_factor_df`/`apply_adj_factor*`/旧 `bar_repository.get_bars`，禁止业务层自行 resample 周/月频率，禁止除 MDAS 外导入 `kline_aggregator`，禁止除 MDAS 外导入 repository 私有行情查询函数。（9）**quote 边界**：`/quote` 仅服务顶部行情卡片实时报价，可作为独立实时报价出口，但不得作为历史 K 线或指标输入；前端主/小 K 线显式请求 completed qfq bars，实时价格只由 quote overlay 呈现，禁止后端 partial bar 与 quote 重复合并。（10）**603538 真实回归 + PINE_PARITY_PENDING**：美诺华 603538（instrument_id=`1fea317d-7206-41e9-b371-2ef79a57ce73`，除权日 2026-07-09 价格从 39.33 跳到 28.07）作为除权真实回归样本，600276 恒瑞医药作为无公司行为对照（factor 全 1.0）；SMC 仅验证输入一致和无回归，不宣称 Pine 输出级完全对齐，保留 `PINE_PARITY_PENDING`（待 TradingView CSV fixture）。

***

57. docs 顶层目录结构规范（CHANGE-20260718-002）：（1）**只允许四个子目录**：`docs/` 直属子目录只允许 `current/`（当前事实）、`maps/`（实现地图）、`changes/`（过程证据/CHANGE records）、`archive/`（历史快照）；`docs/` 根 `.md` 文件（如 README.md）不受限。（2）**禁止新建非规范顶层目录**：禁止新建 `analysis/`、`architecture-audits/`、`design/`、`notes/` 等非规范顶层目录；分析类文档归入 `maps/`（实现地图）/ `changes/`（过程证据）/ `current/`（当前事实）/ `archive/`（历史快照），不得新建顶层目录制造重复事实源。（3）**强制校验**：`tools/check_docs_consistency.py` 规则 11 `check_unauthorized_top_level_dirs()` 拒绝任何未授权顶层目录；CI/质量门禁必须通过该检查。（4）**CHANGE 引用可达性**：`check_change_references()` 规则 12 校验所有 `docs/` 递归 + `AGENTS.md` 中 `CHANGE-YYYYMMDD-NNN` 引用对应 `docs/changes/records/` 文件存在；引用已删除或未创建的 record 即 FAIL（`archive/` 历史快照豁免）。（5）**不制造重复事实源**：有效文档迁移时归入规范目录而非简单移入 archive；历史引用（CHANGE records / CHANGELOG 内原路径）保留不可变。

58. Docker 构建性能与磁盘治理规范（CHANGE-20260718-003）：（1）**版本 ARG 必须在依赖层之后**：所有 Dockerfile 中 `ARG GIT_SHA`/`ARG BUILD_TIME`/`ARG BUILD_TIME` 等版本参数必须声明在 `apt-get install`/`pip install`/`npm ci`/`COPY` 等依赖与源码层之后，仅 `ENV`/`LABEL` 等 metadata 层受版本变化影响；ARG 值变化会使后续所有层缓存失效（即使 RUN 不引用 ARG），故版本 ARG 绝不可放在依赖层之前。（2）**BuildKit cache mount**：pip 安装使用 `RUN --mount=type=cache,target=/root/.cache/pip`，npm 安装使用 `RUN --mount=type=cache,target=/root/.npm`；所有 Dockerfile 首行 `# syntax=docker/dockerfile:1.4`；禁止 `PIP_NO_CACHE_DIR=1`（改用 cache mount 持久化下载缓存）。（3）**基础镜像 digest 固定**：`python:3.11-slim`/`node:20-alpine`/`nginx:alpine` 必须用 `@sha256:...` digest 固定；构建默认不 `--pull`，仅基础镜像维护时显式更新 digest；非本地镜像（如 playwright）可暂用 tag 但须注释说明维护时固定 digest。（4）**json-file 日志轮转**：`docker-compose.prod.yml` 所有服务必须配置 `logging`（`json-file`，`max-size: 50m`，`max-file: 5`，单容器上限 250MB）；`/etc/docker/daemon.json` 配置系统级默认 `log-driver`/`log-opts` + `builder.gc`。（5）**cleanup 脚本保留策略**：`scripts/cleanup-docker.sh` 按 `KEEP_VERSIONS=2`（当前 + 1 rollback）保留 `market-dev-*` 镜像，移除 7 天过滤，同日处理无引用 SHA 标签与 build cache；`PROTECTED_REPO_PREFIXES` 保护基础镜像（python/node/nginx/postgres/redis/playwright）绝不删除；rmi 前检查运行容器使用；禁止 `docker system prune -a`/volume prune/删除生产数据。（6）**热构建不重跑依赖安装**：仅改代码的热构建 `pip install`/`npm ci` 层必须 CACHED（依赖声明文件不变即缓存），验证标准为构建日志显示 `CACHED` 且耗时 <10s。

59. ref/ 目录彻底隔离硬规则（CHANGE-20260718-004）：（1）**ref/ 仅人工阅读**：`ref/` 目录下所有文件（含 `ref/smc_user_source.pine`、`ref/smc_user_export.pine`、`ref/smc.py`、`ref/盘迹品牌视觉资产包_v1.0/` 等）仅供人工阅读参考，**禁止作为运行依赖**；生产代码、测试、工具、构建脚本在运行时不得 `import`/`open`/`read`/`glob` `ref/` 目录下任何文件。（2）**禁止称为真源/合同/fixture 生成器**：`AGENTS.md`、`docs/current/*.md`、`docs/maps/*.md` 不得把 `ref/` 文件称为"真源"、"合同"、"fixture 生成器"或"运行依赖"；可称为"参考源（人工阅读）"或"历史路径"；算法真源必须是生产代码（如 `smc_pine_core.py`、`node_cluster_engine.py`、`indicator_contract.py`、`indicator_semantics.py`）。（3）**git 跟踪状态**：`ref/smc_user_source.pine`（用户原创 Pine 源码，SHA256 0bd3d2ad，843 行）保留 git 跟踪（clause 46 明确要求，`git add -f` 例外）；`ref/smc_user_export.pine`（派生文件，source 末尾加 18 个隐藏 plot）已于 CHANGE-20260718-004 `git rm --cached`，不再纳入 git 跟踪，`.gitignore` 的 `ref/` 规则自动忽略。（4）**历史 CHANGE/archive 例外**：`docs/changes/records/*.md` 和 `docs/archive/**` 允许保留历史 ref 事实（如旧路径引用、SHA256 记录），作为不可变历史证据，不作为当前权威；当前权威以 `docs/current/` 和 `docs/maps/` 为准。（5）**架构守护测试**：`backend/tests/test_ref_isolation.py` AST/文本门禁扫描 `backend/app/**/*.py` + `backend/tests/**/*.py`（排除 fixtures）+ `tools/**/*.py`，禁止 `from ref...`/`import ref...`/`open("ref/...")`/`Path("ref/...")`/`glob("ref/...")`；`docs/current/*.md` + `docs/maps/*.md` + `AGENTS.md` 文本扫描禁止 "ref/...是真源"/"ref/...作为运行依赖" 表述；`git ls-files ref/` 不得包含 `ref/smc_user_export.pine`。

60. SMC 测试 fixture 来源单一（CHANGE-20260718-004）：（1）**CSV fixture 唯一来源**：SMC Pine parity 测试只读取 `backend/tests/fixtures/smc_pine/*.csv`；CSV 由用户在外部从 TradingView 手工导出提供，**项目不依赖 `ref/` 导出脚本**（`ref/smc_user_export.pine` 仅人工阅读，不在 git 跟踪，不作为测试输入）。（2）**fixture 路径规范**：`backend/tests/fixtures/smc_pine/smc_tv_<symbol>_<tf>.csv`（如 `smc_tv_000725_1d.csv`）；fixture 缺失时 `test_smc_tv_parity.py` 自动 skip 并标记 `PINE_PARITY_PENDING`，**不得宣称"完全对齐"**（详见 clause 53）。（3）**禁止从 DB 重新取 bar**：TV parity 测试直接读取 CSV fixture，禁止从数据库重新取另一套 bar 与 TV 比较（clause 53 已确立）。（4）**fixture 不可变**：一旦用户提供 CSV fixture 并 commit，禁止修改 fixture 内容；fixture SHA256 记录在 `backend/tests/fixtures/smc_pine/README.md` 中作为不可变事实。（5）**000725 项目回归基线**：在 CSV fixture 提供前，京东方A 000725 的 SMC 输出（17 events / 21 OB / 2 EQL / swing_bias=1）作为项目回归基线（clause 53 第 5 项），用于验证算法无回归，不用于宣称 TV 对齐。

61. 全算法族 SSOT 与原子渲染长期规则（CHANGE-20260718-006）：（1）**样本股只能用于验证，不能限定修复范围**：000725/603538/利通电子等样本股仅作为算法回归基线与渲染一致性验证样本，禁止把修复范围限定为"只让某只股票通过"；任何算法/合同/渲染缺陷必须按算法族/合同维度根治，覆盖全部股票，禁止"为样本股量身定制"。（2）**每个算法族只能有一个 Kernel**：12 个算法族（`node_cluster`/`dsa`/`smc`/`bollinger`/`macd`/`sqzmom`/`breakout`/`participation`/`temporal_features`/`structural_features`/`primary_secondary_relation`/`snapshot_derived_features`）必须通过 `AlgorithmRegistry`（`backend/app/contracts/algorithm_registry.py`）注册唯一 Kernel，禁止同一算法族存在多个计算入口；AST 守护测试 `backend/tests/test_algorithm_registry_architecture.py` 强制约束；Kernel 路径见 `docs/current/08-indicator-calculation-contracts.md` Section 5.1。（3）**四条链只能使用统一计算网关**：详情/盘后/盘中/Capture 四条调用链必须通过 `CanonicalComputationService`（`backend/app/services/canonical_computation_service.py`）调度已注册算法，禁止生产模块直接 `import` kernel 绕过注册表；四条链只能做适配（节奏/去重/TTL/截图），基础指标值必须来自同一 Kernel；相同输入（instrument + timeframe + as_of + source_bar_hash + adj_factor_hash）必须得到相同 `result_hash`（5 维度确定性）；四链地图见 `docs/maps/indicator-computation-map.md` Section 1-5。（4）**Bars 与 Indicators 必须形成原子渲染帧**：前端 `ChartRenderFrame`（`frontend/src/utils/chartRenderFrame.ts`）作为周期切换原子渲染门禁，bars 帧与 indicators 帧不匹配时跳过指标渲染（显示"指标加载中..."横幅）；纵轴 domain policy（`computeVisiblePriceBounds` + `shouldIncludeNodeInPriceRange` + `shouldIncludeSmcTrailingInPriceRange`）过滤远端 Node/trailing 不参与纵轴候选；禁止周期切换时显示上一周期残留指标或远端 Node 撑爆 y-range。（5）**要求图片时，只有文字成功不算整体成功**：飞书状态机中，要求图片的投递必须 `card_status=success` 且 `image_status=success` 才整体 `success`；`card_status=success` 但 `image_status!=success` 时必须标记 `failed`（图片确定性失败，`image_definitively_failed`）或 `pending`（图片仍在进行中），禁止用 `partial_failed` 或 `success` 掩盖图片失败；`image_definitively_failed` 判定覆盖 capture 失败 / `image_delivery` failed/dead / `image_upload_status=failed`；测试覆盖 `backend/tests/test_state_machine.py` / `test_stock_detail_feishu_status.py`。（6）**复权算法版本变化必须触发全市场一致性审计**：`factor_algorithm_version` 或 `reconciliation_version` 变化时必须触发全市场复权因子一致性审计（只读审计 → 小批次串行重建），禁止局部修补或仅更新最新 5 根；公司行为集合或 fingerprint 变化时必须从最早受影响日期重新计算该股票完整日线 factor 序列并原子 upsert；因子失败不得用 1.0 伪装成功，必须返回 `degraded` 及原因；审计与重建入口见 `backend/app/services/adjustment_factor_reconciliation_service.py`（CHANGE-20260718-005 Section 1）。

62. 日线 SMC 盘中监控 + 三类独立飞书图片 + Canonical 四链 re-export 接入（CHANGE-20260720-001）：（1）**MarketDataContext 字段拆分**：`MarketDataContext` 必须明确分离 `bars_display`/`display_timeframe`（展示周期）与 `bars_daily`/`bars_15min`/`bars_minute`（计算输入）；Node/BB/SMC daily structure 必须只使用 `bars_daily`；当前周期图层（DSA/MACD/SQZMOM）使用 `bars_display`；禁止 `MarketDataContext.bars_daily=macd_bars` 导致 Node 主输入随展示周期变化（lessons learned：旧实现此赋值导致 Node 主输入被展示周期污染）。（2）**详情 API Node 输入修复**：`/stock/:symbol` 详情 API 必须独立输出 `data.node_cluster`，使用 fixed completed qfq 250 1d + 4000 15m bars；不加载 1m；Node Cluster 主输入独立于展示周期，五周期（1d/15m/1h/1w/1mo）profile_hash 完全一致；`TestNodeClusterFivePeriodConsistency` 覆盖。（3）**SmcMonitor 日线主周期**：新增 `SmcMonitor`（`backend/app/services/smc_monitor.py`），主输入已完成前复权日线，1m 仅触发检测；调用 `canonical_adapters.compute_smc_adapter`；FVG 完全排除；3 个 V1 事件 `smc_bos_retest`/`smc_choch_retest`/`smc_order_block_first_touch`；稳定 `smc_entity_id` + 去重维度含 `instrument/event_type/entity/touch_episode`。（4）**WatchlistMonitor 三合一**：`WatchlistMonitor` 聚合 `self._bb`（BollingerMonitor）+ `self._vn`（VolumeNodeMonitor）+ `self._smc`（SmcMonitor）三个子监控；单子监控失败只标记该项 `degraded` 不阻塞其他子监控。（5）**三监控日线主周期统一**：BB/VN/SMC 三类监控必须使用已完成前复权日线为主结构，15m 仅 Node 成交量分配辅助，1m 仅触发检测；`MarketDataContext.bars_display` 与 `bars_daily`/`bars_15min`/`bars_minute` 分离；禁止 15m/1m 进入主结构计算。（6）**MonitorState 命名空间升级**：`MonitorState` 升级为 `bb`/`node_cluster`/`smc`/`market` 命名空间；单子监控失败只标记该项 degraded 不影响其他项；`EVENT_LABELS` 5→8 项（新增 3 个 SMC 事件）。（7）**三类 IndicatorView 独立飞书图片**：`IndicatorView = Literal["node_cluster", "bollinger", "smc"]` 共享枚举贯穿 `StrategyEvent.payload` → `NotificationMessage.resource_refs` → Capture 请求 → `CaptureJob` → 输出文件名 → 缓存键 → 幂等键 → 状态查询 → 前端 URL 参数；三套 `FEISHU_CAPTURE_PRESETS` dict（`node_cluster`/`bollinger`/`smc`）layers 互斥（除共享 candlestick）；`build_monitor_event_text` 按 IndicatorView 拆分文字卡片；一图一指标禁止混合。（8）**详情页飞书发送指标选择**：`POST /api/v1/stock-detail-feishu` body `SendFeishuRequest` 含 `indicator_view: Literal["node_cluster", "bollinger", "smc"] | None = None` 字段；前端弹窗三单选项（筹码共识价 / 布林带 / 智能资金），单选；`indicator_view=None` 时默认 `node_cluster`；非法值返回 422；`useStockDetailFeishu` hook 支持三选项；前端 URL 参数含 `indicator_view`。（9）**Canonical 四链 re-export 接入**：`backend/app/services/canonical_adapters.py` 作为 SSOT 入口（不在 `_FOUR_CHAIN_MODULES` 中）可自由 import kernel 模块并 re-export 给四链使用；四链模块（`indicator_service` 5 个 / `feature_snapshot_service` 4 个 / `monitor_batch_service` 1 个 / `stock_capture_service` 0 个）禁止直接 import kernel 模块；`compute_macd_adapter` 延迟 import `compute_macd` 规避 `indicator_service → canonical_adapters → indicator_service` 循环依赖；AST 硬门禁 `test_four_chain_no_direct_kernel_import` 从 `xfail(strict=True)` 升级为硬失败；`tests/allowlist.json` 移除 issue #83 xfail 条目。（10）**Capture 缓存键含 indicator_view**：`CaptureJob` 新增 `indicator_view` 字段；缓存键/输出文件名/幂等键含 `indicator_view`；同股票不同 IndicatorView 缓存隔离；状态查询 `get_share_status` 返回 `indicator_view`。

63. FR-11 因子变化下游缓存精确失效 + nodeAvailability 5 态状态机 + 移动飞书指标舞台（CHANGE-20260721-001）：（1）**FR-11 因子变化下游缓存精确失效**：`AdjustmentFactorService.rebuild_factor_series` 成功后必须调用新增 `_invalidate_downstream_caches` 方法精确失效该股票下游三层 Redis 缓存——MDAS（`mdas:{instrument_id}:*`，sync Redis scan+delete）+ bars（`bars:{instrument_id}:*`，async，`bars_redis_cache_enabled` 默认 False 时返回 0）+ indicator（`indicator:{instrument_id}:*`，async，TTL 300s）；**单层失效失败不阻塞其他层**（缓存 TTL 会自然过期），三层失效均不抛异常向上传播，`rebuild_factor_series` 仍返回成功；**不失效**（依赖 TTL 自然过期或重算）：监控 Profile 缓存（`monitor_batch_service._node_cluster_profile_cache` in-process dict，跨 worker 无法精确失效，TTL 300s）、Capture 缓存（filesystem per-event key，新事件自然 miss，TTL 600s）、Snapshot（DB 存储，由 after_close 流水线重算，schema_version bump 保证旧快照不可见）；rebuild 失败时 `_delete_fingerprint` 回滚，可重试；禁止用 1.0 伪装成功；测试覆盖 `test_factor_reconciliation.py::TestInvalidateDownstreamCaches`（4 个测试）。（2）**nodeAvailability 5 态状态机**：StockContext API `GET /api/v1/stocks/{symbol}/context` 新增顶层必填字段 `nodeAvailability`（`NodeAvailabilityInfo` Pydantic 模型，与 `core`/`auxiliary`/`availability`/`productObservations` 并列，**不计入 Core 14 / `availability.coreDenominator`**），从 `snapshot.primary.1d.node_cluster` 提取诊断字段；`reasonCode` 取值：`NO_PUBLISHED_RUN`/`SNAPSHOT_MISSING`/`NODE_PROFILE_EMPTY`/`NODE_15M_MISSING`/`NODE_COMPUTE_FAILED`/`NODE_INSUFFICIENT_DAILY_BARS`/`LEGACY_SNAPSHOT_NO_NODE_CLUSTER`/`null`（available）；`degraded_reason` 到 `reasonCode` 映射稳定，前端按 `reasonCode` 渲染对应降级文案；管理员 debug 接口字段与普通用户一致。（3）**Feature Snapshot schema_version 3→4**：`feature_snapshot_service._SCHEMA_VERSION` 从 3 bump 到 4；`node_cluster` 字段始终写入 `availability`/`degraded_reason`/`profile_hash`/`daily_source_hash`/`bars_15m_source_hash`/`daily_bars_count`/`bars_15m_count` 诊断字段（即使 profile 为 None 也写入最小诊断字段）；旧 schema_version=3 snapshot 不可见（查询按 `schema_version=4` 过滤），保证旧新结果不可混用；回滚方式：将 `_SCHEMA_VERSION` 改回 3 即可恢复旧快照可见；部署后需对最近交易日执行受控全市场 full Snapshot 重算（schema_version=4）。（4）**三链五周期一致性**：详情链 `/stock/:symbol` 切换 1d/15m/1h/1w/1mo 时，`profile_hash`/`daily_source_hash`/`bars_15m_source_hash` 必须完全一致（图表 bars frame hash 允许不同）；Atomic Facts 中的"筹码共识价"与详情页 Node Cluster 必须消费同一个 Canonical 结果（`node_cluster_engine.compute_node_cluster_profile` 唯一入口，三链同核）；测试 `test_node_cluster_three_chain_consistency.py::TestNodeClusterFivePeriodConsistency`（4 个测试）+ `test_stock_context_atomic_facts.py`（9 个 nodeAvailability 测试）+ `test_feature_snapshot_service.py`（4 个 node_cluster 字段写入测试）覆盖。（5）**ChartRenderFrame 帧比对门禁**：前端 `chartRenderFrame.ts` 使用 `displayHash`/`displayRangeKey` 优先，降级到 `sourceBarHash`；展示帧（display_frame）与算法输入诊断（calculation_diagnostics）分离，由 `build_display_frame()` 生成；3 态状态机：pending/success/mismatch-error；bars API 与 indicators API 共用 `build_display_frame()`；测试 `chartRenderFrame.test.ts` 覆盖。（6）**移动飞书舞台 1440×2560 9:16**：新增 `MobileIndicatorStage.tsx`（1440×2560 9:16 移动舞台，`device_scale_factor=1`），移植自 `ref/panji_short_video_integrated_studio_v1_15_event_flash_fix/` 舞台视觉外壳；`CaptureStockPage.tsx` 读取 `indicator_view` URL 参数并包裹 `MobileIndicatorStage`；`StrategyChart.tsx` 新增 `indicatorView` prop + `effectiveLayers` 预设替代默认全图层；`INDICATOR_VIEW_LAYER_PRESETS` 每个 indicator_view 对应一个 `ChartLayerVisibility` 预设；`include_smc` 按需计算（`indicator_view=smc` 时触发 SMC 算法）；测试 `stockResearchTypes.test.ts`（9 个 INDICATOR_VIEW_LAYER_PRESETS 测试）覆盖。

64. Display Frame Contract V2 + Node DTO V2 + 移动舞台 V2 + 四层缓存失效 + 因子审计完整字段（CHANGE-20260721-002）：（1）**Display Frame Contract V2**：`backend/app/services/indicator_display_frame.py` 删除 `_display_window=100` 硬编码，改用请求 bars 参数；`display_frame` 新增 V2 字段（`requested_count`/`actual_count`/`first_time`/`last_time`/`include_realtime`/`is_partial`/`adjustment_as_of`）；`DisplayWindowSpec` dataclass 封装三参数，`to_cache_suffix()` 方法生成缓存后缀（如 `rt=False_co=True_ao=2026-07-18`）追加到 indicator_cache key 末尾，不同 spec 独立缓存；旧 v11 缓存的 display_frame 缺失 V2 字段且展示窗口固定 100 根，必须强制失效；`ALGORITHM_VERSION` v11→v12（CHANGE-20260721-002 Display Frame Contract V2）。（2）**Indicators API V2 参数**：`/api/v1/instruments/{instrument_id}/indicators` 新增与 `/bars` 同款的 V2 参数 `include_realtime`/`completed_only`/`adjustment_as_of`，确保指标计算与 bars 请求使用同一份行情口径；`backend/app/api/indicators.py` 端点透传新参数；`backend/app/services/indicator_service.py` 调用 `build_display_frame()` 时传入请求参数；前端 `frontend/src/api/endpoints.ts` 请求函数透传 V2 参数。（3）**Canonical Node DTO V2**：`node_cluster` DTO 升级为 `node_regions` + 独立 `price_state`；`node_regions` 是节点区域列表（每个区域含 `price_low`/`price_high`/`volume`/`is_poc`/`is_value_area`/`peaks`），`price_state` 是当前价格状态独立字段（`current_price`/`nearest_node_above`/`nearest_node_below`/`position_in_value_area`/`position_pct`）；四链统一消费 V2 DTO（详情/盘后/盘中/Capture）；`canonical_adapters.compute_node_cluster_adapter` 返回 V2 结构；类型特定 Ready 条件（不同 indicator_view 有额外 Ready 条件，如 `indicator_view=smc` 时需 SMC layer 加载完成）；`backend/tests/test_node_cluster_engine.py` V2 DTO 断言更新。（4）**移动舞台 V2 + renderDensity 集中管理**：新建 `frontend/src/components/chartRenderScale.ts` 集中管理 Canvas 字号/线宽/节点标记尺寸；`renderDensity` 两套规格——`desktop`（保持不变）和 `mobile_capture`（放大）；三类缩放规格接口：`ChartTypography`（axisLabel/paneLabel/nodeLabel/riskNotice 等字号）、`ChartStrokeScale`（grid/crosshair/candlestick 等线宽）、`ChartGeometryScale`（nodeMarker/eventDot 等几何尺寸）；`StrategyChart.tsx` 所有 27 处 `drawText` 调用显式传 `scale.fonts.*`（默认 font 改为空串，禁止硬编码 `'10px ui-monospace...'`）；`drawLine` 增加 `scale.strokes.grid` 参数；`CaptureStockPage.tsx` Playwright 截图选择器 `[data-testid="stock-detail-capture"]`；`global.scss` 风险提示字号 30-32px + 透明度 ≥0.72（`opacity:` 属性 或 `rgba(..., α)` alpha 通道均可接受，CSS 等价可读性）；测试 `capture-stock-page.test.ts` Test 23 + Test 25 覆盖；`chartRightPadding.test.ts` 网格线断言改正则匹配 `/drawLine\(ctx, g\.l, y, g\.plotRight, y, C\.grid(?:,\s*scale\.strokes\.grid)?\)/`。（5）**四层缓存失效（FR-11 V2 扩展）**：`AdjustmentFactorService._invalidate_downstream_caches` 返回值从三层 dict 扩展为四层 dict——新增 `capture` 字段；新增 `_invalidate_capture_cache` 方法：按 `instrument_id` 精确扫描 `CAPTURE_CACHE_DIR`（来自 `stock_capture_service._CACHE_DIR`），删除文件名含 `_{instrument_id}_` 的文件（Capture 缓存键格式 `{event_id}_{instrument_id}_{chart_version}_tf=..._sbt=..._run=..._dsf=..._iv=...`）；目录不存在/空返回 0；扫描失败记 warning 不阻塞；TTL 600s 兜底；**单层失效失败不阻塞其他层**；四层失效均不抛异常向上传播，`rebuild_factor_series` 仍返回成功；测试覆盖 `test_bars_scheduler_factor_audit.py` 4 个 Capture 缓存清理测试（`test_invalidate_capture_cache_deletes_only_matching_files`/`test_invalidate_capture_cache_missing_dir_returns_zero`/`test_invalidate_capture_cache_empty_dir_returns_zero`/`test_invalidate_downstream_caches_includes_capture_field`）。（6）**因子审计完整字段集合（PROMPT.md §5.4.2 V2）**：`bars_scheduler_service._audit_and_rebuild_factors` 返回的 summary dict 必须包含完整生产审计字段——`trade_date`（ISO 字符串，如 `"2026-07-18"`）/`total_audited`/`consistent`/`needs_rebuild`/`audit_rebuilt`（本审计阶段实际重建成功数，区分 `_rebuild_factors_if_needed.rebuilt`）/`rebuilt`（向后兼容：等于 `audit_rebuilt`）/`failed`/`errors`/`failed_symbols`（失败股票代码列表，便于生产验证定位）；`_rebuild_factors_if_needed` 返回值同样新增 `trade_date`/`failed_symbols` 字段；rebuild_batch 成功路径同步设置 `audit_rebuilt` 和 `failed_symbols`；rebuild_batch 异常路径设置 `failed_symbols`（所有 needs_rebuild 都视为失败）；测试 `test_bars_scheduler_factor_audit.py` 更新 `test_audit_empty_instruments`/`test_audit_needs_rebuild_all_success` 断言新增字段。（7）**完整测试套件验证（Phase 6）**：后端 599 passed + 14 skipped（含 indicator/capture/node_cluster/after_close/bars/SMC/monitor/screener/architecture 组）；前端 420 passed + 3 pre-existing failed（structural-state-toggle，StockDetailPage 未修改）；Ruff 全绿；Mypy 3 pre-existing errors（`redis_client.py`/`bar_repository.py`，与本次改动无关）；TypeScript 仅 pre-existing errors；ESLint pre-existing 模块损坏（`eslint-visitor-keys`）；架构测试 46/46 PASS；修复 3 个测试（`test_cache_algorithm_version_bumped_to_v11`→`v12`、`test_bars_scheduler_factor_audit` 新增字段断言、`chartRightPadding` 正则匹配）+ 新增 4 个 Capture 缓存清理测试。（8）**不变量**：不新增依赖/表/migration；不重写 SMC 算法；MDAS 仍为唯一行情读取出口；`ref/smc_user_source.pine` 843 行 + SHA256 不变；`PINE_PARITY_PENDING` 保留至用户提供 TV CSV fixture；分支 `fix/display-frame-node-mobile-capture-v2` @ `0132d4b`（from origin/main，PR #86 merge）。
