...
- **如何验证**：运行 `grep -RInE 'sqlite|aiosqlite' backend/tests`，除 ADR/规则引用或 `test_config_validation.py` 中用于校验拒绝 sqlite 的场景外，不得出现 SQLite 相关字符串；Integration 测试必须通过 `APP_ENV=test TEST_DATABASE_URL=postgresql://... pytest` 执行。
...
- **如何验证**：`grep -RInE 'aiosqlite' backend` 无任何命中；`grep -RInE 'sqlite' backend` 仅命中 `config.py` 拒绝校验、`test_config_validation.py` 校验场景、`test_architecture_rules.py` 规则字符串及 ADR 文档说明。
...
- **如何验证**：`grep -RInE 'CREATE TABLE|@compiles' backend/tests` 无任何命中（除规则测试自身引用）。
...
- **如何验证**：`backend/tests/conftest.py` 的 `init_test_db` fixture 必须在 session 范围自动调用 `alembic upgrade head`；`test_architecture_rules.py` 检查测试目录无手写 schema。
...
- **如何验证**：AST 扫描或 `grep -RInE '@pytest_asyncio\.fixture\s*\n?.*\bdb_session\b|def db_session' backend/tests` 仅命中 `conftest.py`；`test_architecture_rules.py` 增加同名 fixture 覆盖检查。
...
- **如何验证**：`grep -RInE 'Role\(.*name\s*=\s*["\x27]user["\x27]|_ensure_role\(.*["\x27]user["\x27]' backend` 无任何命中；`test_architecture_rules.py` 增加角色字符串门禁。

### 规则 8：管理员无套餐、无 subscription

- **规则正文**：`admin` 角色不绑定任何套餐，不创建 subscription 记录，`plan_code=None`，限额无限制。禁止为管理员硬编码 `ADMIN_PLAN_CODE`、`research_50` 或任何套餐信息。
- **理由**：admin 的权限应来自角色本身，而非套餐契约；给 admin 绑定 subscription 会引入事实源冲突，导致限额校验与套餐到期逻辑混乱。
- **违反示例**（历史）：`backend/app/constants/plan_codes.py` 曾定义 `ADMIN_PLAN_CODE = "research_50"`；`backend/app/api/me.py` 曾对 admin 返回 `plan_code = ADMIN_PLAN_CODE`；相关测试曾断言管理员返回 `research_50`。
- **如何验证**：删除 `ADMIN_PLAN_CODE` 常量；`grep -RInE 'ADMIN_PLAN_CODE' backend` 无任何命中；管理员相关测试断言 `plan_code is None`；`test_architecture_rules.py` 增加该常量门禁。
...
- **如何验证**：各业务领域事实源见 `docs/事实源清单.md`；`test_architecture_rules.py` 对 plans 业务值、算法常量等进行重复检测；文档修改后必须运行 `python tools/update_docs.py` 并提交 diff。
...
- **修订流程**：新增、删除或修改硬规则，必须同步更新 ADR 或对应 docs 规范，并运行 `python tools/update_docs.py` 重建受影响的自动生成的文档。
