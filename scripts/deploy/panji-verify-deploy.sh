...
# [CHANGE-20260806-005 Phase 7] 验证栈 host 端口默认避开稳定运行栈：生产 trading-backend
#   绑定 0.0.0.0:8000、trading-frontend 绑定 80，verify 必须用非冲突端口，否则 up 时端口占用失败。
VERIFY_BACKEND_HOST_PORT="${VERIFY_BACKEND_HOST_PORT:-18000}"
VERIFY_FRONTEND_HOST_PORT="${VERIFY_FRONTEND_HOST_PORT:-18080}"
export VERIFY_CODE_DIR VERIFY_RUNTIME_DIR VERIFY_PG_NETWORK="$PG_NETWORK" VERIFY_BACKEND_IMAGE VERIFY_FRONTEND_IMAGE \
  VERIFY_BACKEND_HOST_PORT VERIFY_FRONTEND_HOST_PORT
