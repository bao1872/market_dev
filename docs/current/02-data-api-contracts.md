...
- `value_area_position_0_1` = `(close - val_price) / (vah_price - val_price)`
...
| 字段 | 公式 |
|---|---|
| m15_position_relative_to_daily | `m15_price_position_in_swing_0_1 - daily_price_position_in_swing_0_1` |
...
- V1 only `as_of=latest`，不实现历史回放截断；
- 主周期 lookback = 250 bar，副周期 lookback = 500 bar；
...
- 每个组（daily/m15/derived）独立 try/except，单组失败返回 null + `degraded_reasons`；
