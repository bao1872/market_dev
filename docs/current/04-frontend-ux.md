...
- **详情页来源上下文契约 V2（CHANGE-20260713-009 → DetailSourceContextV2）**：`/market` 导航至 `/stock/:symbol` 时通过 `buildStockDetailUrl` 传递 `originScope` + `sourceRunId`（入口快照 activeRunId）+ `canonicalQuery`（`JSON.stringify(resultParams)`，含 universe/sort_by/sort_desc/keyword/industry/concept/metric_filters/page/page_size）+ `returnTo`（由 `buildMarketReturnToUrl(marketListCtx, symbol)` 从内存状态构建，非 searchParams 副本）；详情页左栏 `useStockDetailActions` 用固定 `sourceRunId` + `canonicalQuery` 调 `useStrategyRunResults`（**禁止 fresh `usePublishedRuns` 推导 activeRunId**），保证列表页与详情左栏请求同一 run + 同一 query → 有序 symbol 数组完全一致；`decodeMarketListContext`/`buildStrategyResultQueryParams` 共享纯函数（`marketWorkspaceUrlState.ts`）由 `MarketWorkspacePage` 和 `useStockDetailActions` 共用避免 filter drift；`sourceListKind` 由 `origin` 派生（`market`→"行情来源"，`watchlist`/`direct`→"自选来源"）；`normalizeInternalReturnTo` 长度限制 4096。**stableContextId 不变性**：`computeStableContextIdV2(origin, sourceRunId, canonicalQueryRaw)` 只由这三者计算，**不含 selectedSymbol、不含 returnTo**（returnTo 含 `selected=入口symbol`，纳入会间接破坏切股不变性）；切股/刷新后 stableContextId 不变。`useWatchlistMonitorStatus` 仅用于 `inWatchlist` 状态，**禁止充当来源列表数据源**。
...
- **详情页左栏 loading 占位契约（CHANGE-20260715-004）**：`useStockDetailActions` 新增 `sourceListLoading: boolean` 字段（V2：`origin` 有效时为 `sourceResultsQuery.isLoading`；`origin` 无效时为 `monitorStatusQuery.isLoading`；**禁止用 `!activeRunId` 作为永久 loading**，V2 用固定 `sourceRunId` 替代 fresh `usePublishedRuns`）；`StockDetailPage` 左栏渲染分两个分支——`sourceListLoading=true` 时渲染 `<aside data-testid="detail-source-list-loading" class="tv-source-list tv-source-list-loading">`，内含 header（`detailActions.sourceListKind === 'market'` ? "行情来源" : "自选来源"）和 `<div class="tv-source-list-placeholder">加载中…</div>` 占位文案；`!sourceListLoading && sourceStocks.length > 0` 时渲染正常列表（`<aside data-testid="detail-source-list" class="tv-source-list">`）；CSS `.tv-source-list-placeholder { padding: 16px 10px; font-size: 12px; color: #778297; text-align: center; }`；用户从 `/market` 进入 `/stock/:symbol` 后左栏不再空白一段时间才出现列表，而是先显示 loading 占位再切换到实际列表。
- **详情页左栏来源列表切股稳定性（CHANGE-20260725-002）**：`StockDetailPage` **禁止页面级 `instrumentQuery.isLoading`/`!instrumentQuery.data` 早退**（早退会卸载 `tv-detail-layout` 和 `tv-source-list`，切股时闪烁）；`tv-detail-layout` 和来源列表 `aside` 始终挂载，loading/error 仅影响右侧详情和顶部信息栏；来源列表属于 Persistent Source Context，不属于 selected instrument loading；顶部信息栏条件化渲染（`inst` 存在显示股票名称，`instrumentLoading` 显示"加载中..."，其他显示"未找到股票"），新数据返回前不得显示上一只股票的名称/quote/K线/指标；来源列表 aside 容器**禁止 `key={symbol}`**（保持同一 DOM 节点，scrollTop 不丢），列表项保持 `key={s.symbol}`；来源列表渲染条件不依赖 `instrumentQuery` 状态；备忘录按钮 `disabled={!instrumentId}`（inst 未就绪时不可点击）。合同测试 `sourceListStabilityContract.test.ts` 验证 10 项反闪烁不变量。
- **详情页导航 originScope 单一真源（CHANGE-20260716-006）**：`originScope=market|watchlist` 是详情页左栏来源列表的**第一来源真源**，不被 `returnTo.scope` 覆盖；`returnTo` 仅承担返回导航和筛选恢复，不再决定来源类型。统一 URL builder 集中于 `frontend/src/features/stock-research/stockDetailNavigation.ts`，提供 `buildStockDetailUrl` / `resolveStockDetailOrigin` / `sourceForOriginScope` / `strategyForOriginScope`；**三入口统一**：`MarketWorkspacePage` 点击股票、`useStockDetailActions` 上一只/下一只、`StockDetailPage` 左栏来源列表点击，全部调用同一 `buildStockDetailUrl`，禁止手工字符串拼接；旧 `detailNavigation.ts:buildStockDetailUrl` 已删除（**禁止第二套 URL 拼接**）。`originScope` 优先级：显式 `originScope` 参数 > 旧 URL 兼容解析 `returnTo.scope` > 默认 watchlist。**冲突检测**：`originScope` 存在且 `returnTo.scope` 不同 → `contextMismatch=true`，禁止静默回退自选。`MarketWorkspacePage` 生成 returnTo 时使用 `buildMarketReturnToUrl(marketListCtx, symbol)` 从内存 `marketListCtx` 构建（含 scope + selected + 完整筛选/排序/分页），禁止直接复制可能滞后的 `location.search`/`searchParams` 副本。V2 同时透传 `sourceRunId`（入口快照 activeRunId）+ `canonicalQuery`（`JSON.stringify(resultParams)`），确保详情左栏与列表页请求同一 run + 同一 query。
- **详情页左栏来源列表 missing_origin invalid 占位（CHANGE-20260725-003）**：`resolveDetailSourceContextV2` 缺 `originScope` 且无有效 `/market` `returnTo` 时**不再默认 `direct`**（旧默认 direct 会让左栏静默消失为单列），改为 `missing_origin` invalid 占位（`origin=watchlist` 占位 + `contextInvalid=true` + `invalidReason="missing_origin"`），左栏渲染 invalid 占位而非单列布局。**只有显式 `originScope=direct` 才使用单列布局**（合同 5：显式 direct → 单列无左栏；其他场景必须显示左栏 invalid 占位或正常列表）。invalid 占位显示具体 reason：`StockDetailPage` 左栏 invalid 分支渲染 `data-invalid-reason` 属性 + `INVALID_REASON_LABELS` 中文文案（`missing_origin` → "缺少来源上下文"等），禁止空占位。V1 `resolveDetailSourceContext` / `resolveStockDetailOrigin` 已标记 `@deprecated`，**V2 `resolveDetailSourceContextV2` 为唯一真源**（禁止新增 V1 调用）。`market`/`watchlist` 入口必须携带完整 `sourceRunId` + `canonicalQuery`（`buildStockDetailUrl` 强制透传），缺任一字段视为 missing_origin invalid。合同测试 `sourceListVisibilityContract.test.ts` 验证 31 项合同（missing_origin invalid、direct 边界、入口矩阵、防闪烁、V2 唯一真源）。
...
- 个股详情 K 线实时状态以 `/bars` 返回的 `data_source/is_partial/last_live_bar_time/as_of` 为准；**禁止 `mergeRealtimeQuoteIntoBars()` 或任何前端 quote→bar 兜底**（CP-V3-D2 修正：前端不得用 quote 合成/修改 bar）；daily_fallback / 延迟 / 降级行情只用于顶部报价 fallback/状态提示，不混入 `displayBars`；`baseBars` 仍用于指标计算，避免污染算法输入；
- **主/小 K 线统一 qfq completed bars + quote overlay 边界（CHANGE-20260717-002）**：主 K 线（`/stock/:symbol`）和小 K 线（五周期 15m/60m/日/周/月）均显式请求 completed qfq bars（经 MDAS 唯一出口，复权只应用一次）；`none`/`qfq` 切换由 `/bars?adj=` 参数驱动，除权日前后无异常断层。**quote 只做展示 overlay**：实时价格只由 quote overlay 呈现，**禁止后端 partial bar 与 quote 重复合并**；`/quote` 允许独立实时报价出口，但不得作为历史 K 线或指标输入。跨调用方一致性：同一股票/周期/截止日下，`/bars`、indicator/SMC、strategy_batch、feature_snapshot 的 `source_bar_hash`/`adj_factor_hash` 必须一致。
...
- **截图页（CaptureStockPage）实时链路**：
  ...
  - 后端 partial daily bar 为真（`/bars?timeframe=1d&include_realtime=true` 返回 `is_partial=true`）；**禁止前端 `mergeRealtimeQuoteIntoBars()` 或任何 quote→bar 兜底**（CP-V3-D2 修正：Exchange→MDAS→ChartSnapshot→Canvas 为唯一 K 线链）。
...
- **视觉参考源（人工阅读）**：`ref/盘迹品牌视觉资产包_v1.0/`（ref 路径不作为运行时依赖，仅供设计参考）。
