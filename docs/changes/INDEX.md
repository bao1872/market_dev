...
| CHANGE-20260802-003 | 2026-08-02 | 部署执行入口收敛为两脚本（Live Mount）+ CI 降级为手工诊断工具 + Change ID 治理修复 | architecture+ops+governance | 部署脚本/GitHub Actions/部署纪律（业务代码零改动） | 生效（脚本与 workflow 已改；`needs_deploy`：新脚本尚未在 panji-prod 实跑） | SR-01～SR-62 | `rules/80-deployment-data-safety.md`、`maps/80-system-runtime.md`、`runbooks/development-deployment.md` |
| CHANGE-20260802-002 | 2026-08-02 | 竞价归入复盘权限 + 邀请码权限回显 | behavior/contract | 权限/前端 | 历史（代码已合入 dev；Fast CI 已跑，详见对话报告） | PA-12/PA-20/PA-21 | `maps/60-permissions-admin.md` §11 |
...
| CHANGE-20260802-002 | 2026-08-02 | 竞价归入复盘权限 + 邀请码权限回显 | behavior+contract | 竞价/权限 | 已实现，生产外部竞价真值源仍阻断 | AU/PA | `maps/75-auction-analysis.md`、`maps/60-permissions-admin.md` |
| CHANGE-20260802-003 | 2026-08-02 | 部署入口收敛、手工 CI 与 Live Mount 当前合同 | architecture+ops+governance | 部署脚本/CI/规则/Runbook | 代码与本地合同待最终验证；未执行生产部署 | SR-12~14/TQ-90/DS-90~93 | `maps/80-system-runtime.md`、`runbooks/development-deployment.md` |
