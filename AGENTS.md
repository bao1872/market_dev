...
8. 三栏统一行情工作区（阶段三+阶段四+最终对齐）：`/market` 渲染 `MarketWorkspacePage`（左列表 `MarketInstrumentPane` + 中K线 `StockResearchWorkspace` + 右解释面板 `ResearchContextPanel` 可收起）；`/stock/:symbol` 渲染 `StockDetailPage`（路由适配器，复用 `useStockResearchData` + `StockResearchWorkspace`）；`useStockResearchData` 只保留 bars/indicators/quote/events 核心查询，详情页专属能力（自选/上下切换/memo/飞书）拆到 `useStockDetailActions`/`useStockDetailFeishu`；旧 `WatchlistPage.tsx` 和 `IndexPage.tsx` 已删除（统一行情工作区改造），`/overview`、`/watchlist` 仅保留兼容重定向。
...
11. URL 状态保留：`/market` URL 的 scope/symbol/timeframe/source/strategy/event_id/debug/returnTo 进入 URL（可分享、刷新恢复）；切换周期、切换 scope、选择新股票时必须保留其他字段；选择新股票时清除旧 `event_id` 和 `returnTo`（保留 `debug`）；`event_id` 由 `ResearchContextPanel` 消费（事件详情/通俗解释/关键证据）；`debug=1` 仅 `accessProfile.is_admin` 生效（显示原始 factor/feature/JSON），普通用户即使手工加 `debug=1` 也不可见；`returnTo` 为来源页 URL，返回按钮优先使用。
...
16. `StockDetailPage` 的 `event_id` 消费已由 `ResearchContextPanel` 统一实现（最终对齐确立）：`/market` 右栏 `ResearchContextPanel` 在 `event_id` 存在时调用 `useStrategyEventDetail` 显示事件时间、类型（通俗解释）、关联价格、关键证据；无 `event_id` 时展示 `useStructuralFactors`/`useTemporalFeatures` 的人类可读总结；面板关闭（`rightPanelCollapsed`）时不挂载 hooks、不发请求；普通用户不显示内部字段名、算法参数、JSON 或商业机密。
17. 研究上下文面板与管理员调试（最终对齐确立）：`features/research-context/` 含 `ResearchContextPanel`/`EventExplanationCard`/`StructureSummaryCard`/`AdminFactorDebugPanel`/`useResearchContext`；`useResearchContext` 复用现有 `useStrategyEventDetail`/`useStructuralFactors`/`useTemporalFeatures`（React Query 按 queryKey 去重，不产生重复请求）；管理员调试 `AdminFactorDebugPanel` 仅 `accessProfile.is_admin && URL debug=1` 显示原始 factor/feature/timeframe/bar_time/版本/数据源/warmup/degraded 信息和可折叠 JSON；`StockStructuralStatePanel` 新增 `debug?: boolean` props，debug 时显示原始 structural-factors JSON；不新增后端算法，接口缺失时停止该小项并报告，禁止伪造数据；`ScreenerPage` 查看详情进入 `/market?scope=market&symbol=...&source=selection&strategy=dsa_selector&returnTo=...`，`MessagesPage` 有股票时进入 `/market?...&event_id=...`。
