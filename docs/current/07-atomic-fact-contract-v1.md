...
## 9. NodeAvailabilityInfo Schema（CHANGE-20260721-001）

**问题背景**：旧版本 StockContext 对所有 Node 不可用场景统一返回"筹码共识价暂不可用"，无法区分原因（无 run / snapshot 缺失 / profile 空 / 15m 缺失 / 计算失败 / 数据不足 / 旧 schema）。Atomic Facts 中的"筹码共识价"与详情页 Node Cluster 没有共享同一个 Canonical 结果。

**修复方案**：盘后 Snapshot 必须写入 Canonical Node Cluster 诊断字段，StockContext 新增 `nodeAvailability` 字段输出 5 态状态机。
...
class NodeAvailabilityInfo(BaseModel):
    """Node Cluster 可用性诊断信息（CHANGE-20260721-001）。

    与 Core 14 / Auxiliary / ProductObservations 平行，不计入 availability.coreDenominator。
    """
    state: Literal["available", "unavailable"]
    reasonCode: str | None  # 见 §9.2 5 态状态机
    pocPrice: float | None
    profileHash: str | None  # 三链一致性断言用
    dailySourceHash: str | None
    bars15mSourceHash: str | None
    algorithmVersion: str | None  # nc-v1
    dailyBarsCount: int | None
    bars15mCount: int | None
...
class AtomicFactsContextResponse(BaseModel):
    ...
    asOf: date
    core: list[PublicAtomicFactItem]
    auxiliary: list[PublicAtomicFactItem]
    availability: AtomicFactsAvailability
    recentChanges: list[RecentChangeItem]
    dataQuality: DataQuality
    productObservations: ProductObservations | None  # CHANGE-20260716-006
    nodeAvailability: NodeAvailabilityInfo  # CHANGE-20260721-001 必填
```

### 9.2 5 态状态机（reasonCode）

| reasonCode | 触发条件 | dataQuality 记录 |
|---|---|---|
| `NO_PUBLISHED_RUN` | 该股票无任何 succeeded+published full snapshot run | `degradedReasons` 含 `no_published_full_run` |
| `SNAPSHOT_MISSING` | run 存在但 snapshot 行缺失（按 `source_run_id == run.id` 精确查询返回空） | `degradedReasons` 含 `snapshot_missing` |
| `NODE_PROFILE_EMPTY` | snapshot 存在但 `node_cluster.profile` 为空 | `degradedReasons` 含 `node_profile_empty` |
| `NODE_15M_MISSING` | snapshot 存在但 `bars_15m_source_hash` 缺失 | `degradedReasons` 含 `node_15m_missing` |
| `NODE_COMPUTE_FAILED` | snapshot 存在但 `node_cluster.availability=unavailable + degraded_reason=COMPUTE_FAILED` | `degradedReasons` 含 `node_compute_failed` |
| `NODE_INSUFFICIENT_DAILY_BARS` | snapshot 存在但 `daily_bars_count < 250` | `degradedReasons` 含 `node_insufficient_daily_bars` |
| `LEGACY_SNAPSHOT_NO_NODE_CLUSTER` | snapshot `schema_version < 4`（旧 schema 无 node_cluster 诊断字段） | `degradedReasons` 含 `legacy_snapshot_no_node_cluster` |
| `null`（state=available） | `node_cluster.availability=available` + `poc_price` 非空 | 无 |
...
- **GET 请求零写入**：仅 `SELECT`，不写 DB；管理员接口同。

### 9.4 测试覆盖

- `test_stock_context_atomic_facts.py`：9 个 nodeAvailability 状态机测试（覆盖全部 5 态 + legacy + admin debug + 普通用户字段一致）
- `test_feature_snapshot_service.py`：4 个 node_cluster 字段写入测试（available/missing_15m/insufficient_daily/engine_raises）
- `test_node_cluster_three_chain_consistency.py`：4 个五周期一致性测试（profile_hash/profile_rows/independent_of_display_timeframe/degraded_state_when_15m_missing）
