...
`GET /api/v1/instruments/{instrument_id}/structural-factors` 返回双周期（默认 1d + 15m）5 组结构状态因子，由后端 `app.services.structural_factor_service.compute_structural_factors` 统一计算，前端只消费 DTO，不重新计算。无认证要求（与 indicators API 一致）。
...
```json
{
  ...
  "relation": { "trend_alignment": str|null, "momentum_alignment": str|null, "notes": [str] },
  "meta": {
    "as_of": "ISO time",
...
| 因子组 | 字段 | 说明 |
|---|---|---|
| `dsa_segment` | `segment_id`/`segment_dir`/`segment_start_price`/`segment_start_bar_index`/`age_bars`/`segment_extents_pct` | 当前 DSA 段质量；从 `visual_segments[-1]` + `factor_per_bar.regime_id` 推导 |
| `swing_position` | `confirmed_swing_high`/`confirmed_swing_low`/`bars_since_swing_high`/`bars_since_swing_low`/`swing_high_to_close_pct`/`swing_low_to_close_pct` | 已确认 Swing 结构位置（无未来函数） |
| `cost_position` | `poc_price`/`nearest_upper_node`/`nearest_lower_node`/`position_0_1`/`close_to_poc_pct` | Volume Profile / POC / Node |
| `volatility_momentum` | `bb_percent_b`/`bb_bandwidth_pct`/`bb_bandwidth_percentile`/`sqzmom_val`/`sqzmom_delta_1`/`sqzmom_percentile` | Bollinger Bands + SQZMOM_LB |
| `participation` | `volume_ratio_20`/`volume_percentile_120` | 成交参与 |
