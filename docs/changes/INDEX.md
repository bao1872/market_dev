...
| CHANGE-20260803-002 | 2026-08-03 | 权限模型 V2 后端合同收口：统一 grant_days 授权 + 场景化物化 + 用户行锁 + tombstone 撤销 + 结构化审计 + 商业状态 fail-closed + Admin Schema/reason | behavior+contract | 权限/管理员 API/Schema | `verified_local`（纯单元 19 + 定向 56 passed，Ruff/Mypy/检查器通过）；PG 集成待授权轮次 | `prd/60-permissions-admin.md`（新增 §7 PV2-B01~B07） | `maps/60-permissions-admin.md`（新增 §14） |
